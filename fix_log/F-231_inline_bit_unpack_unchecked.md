...
| 일시 | 상태 | 내용 |
|---|---|---|
| 2026-08-14 | 확인 | `verify.html` 인라인 스크립트에 시프트·마스크 언팩을 메모리 주입한 결과, 두 웹 출구가 모두 종료 코드 0으로 통과함을 재현했다. |
| 2026-08-14 | 수정완료 | HTML 인라인 스크립트를 정적 모듈과 같은 입력으로 추출해 시프트·숫자 마스크와 숫자 키 한국어 매핑을 검사하도록 했다. 인라인 결함 재주입 시 두 웹 출구가 모두 실패했다. |
