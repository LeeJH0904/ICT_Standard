...
| 일시 | 상태 | 내용 |
|---|---|---|
| 2026-08-18 | 확인 | **판정**: 0937 6.2는 수정·삭제를 요구하나 DMS는 수집·조회 참조 구현으로 수정·삭제 경로가 없음(불변 감사 이력을 택한 설계 결정, 개발 대조표 6.2-3 `⚠ 부분`). 이는 의도된 선택 적용 → **코드가 아니라 README가 과대표기**. README 범위 정정으로 처리 |
| 2026-08-18 | 수정완료 | 제출본 `README.md` §1.1 범위표 6.2 DMS 행에 적용(등록·연결·조회)/미적용(**수정·삭제 — 불변 감사 이력 설계 결정**) 명시 |
| 2026-08-18 | 수정완료 | **적용 범위**: 제출본 `README.md`에 적용. 이후 사용자 지시로 **저장소 루트 `README.md`에도 동일 §1.1 범위표 이식**. `dms.py` 코드 변경 없음(의도된 부분 적용) |
