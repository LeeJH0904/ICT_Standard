...
| 일시 | 상태 | 내용 |
|---|---|---|
| 2026-08-14 | 확인 | 헬퍼가 실행 폼을 반환하고 `pendingCardHtml()`이 호출하도록 메모리 결함을 주입한 결과, 두 웹 출구가 모두 종료 코드 0으로 통과함을 재현했다. |
| 2026-08-14 | 수정완료 | 공통 소스 검사기가 `pendingCardHtml()`에서 도달 가능한 로컬 헬퍼를 재귀 추적해 실행 버튼·폼 action·`/execute` 경로를 차단하도록 했다. 같은 헬퍼 결함 재주입 시 두 웹 출구가 모두 종료 코드 1로 실패함을 확인했다. |
