...
| 일시 | 상태 | 내용 |
|---|---|---|
| 2026-08-13 | 확인 | `fcs.execute()`가 미승인 규칙의 게이트 판정 전에 제어 프레임을 보내도록 주입했다. 실제 TX가 3건 발생했지만 `gate_e2e.py`는 19/19, exit 0으로 통과했다. |
| 2026-08-14 | 수정완료 | `gate_e2e.py`의 모든 거부 시나리오에서 요청 직전·직후 `link.stats()['tx']`를 대조해 증가 0건을 독립 판정한다(27/27). 동일한 거부 전 송신 변이를 자동 회귀에 고정했으며 TX 3건 주입 시 출구가 실패함을 확인했다. |
