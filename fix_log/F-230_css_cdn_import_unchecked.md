...
| 일시 | 상태 | 내용 |
|---|---|---|
| 2026-08-14 | 확인 | `app.css`에 외부 `@import`를 메모리 주입한 결과, 두 웹 출구가 모두 종료 코드 0으로 통과함을 재현했다. |
| 2026-08-14 | 수정완료 | 실제 로드되는 CSS 전량을 두 웹 출구 입력에 넣고 외부 `@import`와 `url(http/https)`를 검사한다. 두 형태의 회귀 테스트와 결함 재주입이 모두 검출됨을 확인했다. |
