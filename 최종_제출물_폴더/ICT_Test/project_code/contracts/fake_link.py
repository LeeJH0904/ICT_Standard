...
규칙), 이 파일은 그 Protocol을 구조적으로 만족하는 예시일 뿐
production 경로에 배선되지 않는다.

project_code/ 전체에서 합성 데이터(난수·주기함수로 만든 가짜
수치)를 금지한다. 이 대역체는 상태를 갖지 않는 빈 registry로 시작하며, 어떤
수치도 꾸며내지 않는다. 실제 프레임 흐름 검증은 골든 벡터를 재생하는
sim/replayer.py(단계 4)의 몫이다.

`FakeFrameBuilder`도 둔다. `FrameBuilder` Protocol도
`contracts/siap_iface.py`에 있어 `backend/tests/`가 실제
