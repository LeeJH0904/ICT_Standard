...
class Subtype(IntEnum):
    """항목 = TTAK.KO-10.1369-Part1 6.3.3(센서) / 6.3.4(액추에이터).
       코드값 = 표 7-14 각주가 참조하는 [RUCFS-0009] 미확보로 본 구현이 할당."""
...
# 노드당 디바이스 상한 — 0943 미규정, 본 구현이 16 으로 확정. N=16 은 501 byte
# 최대 프레임(RSC+NODE_PROPERTY+DEVICE_PROPERTY×16)과 Timeout≥2×wire_time 산식의
# 전제다. C 대응은 firmware/core/siap_types.h 의 SIAP_MAX_DEVICES_PER_NODE.
...
class Header:
    ...
    trans_type 을 TransType 으로 선언하면 표 7-6 미정의값(0x03)을 담을 수 없어
    "예외를 던지지 않는다"는 계약과 충돌한다. msg_type 이 이미 raw int 인 것과
    같은 원칙으로 trans_type 도 raw int 로 둔다. 해석은 resolve_trans_type() 이
    담당한다."""
...
class DeviceProperty:         # 표 7-15
    """표 7-15는 아래 5개 필드의 타입을 "USER DEPENDENT"로만 규정하고 선택 규칙을
    정하지 않는다(0943 미규정). 이들은 main.value 와 같은 물리량의 경계·정밀도이므로
    **main.value_type 을 따른다**로 결정했다. 코덱은 반드시 이 규칙으로 인코딩·디코딩한다."""
    main: DeviceMainInfo
...
def element_count(kind: MsgKind, payload_len: int) -> int | None:
    ...
    if n == 0 and fixed == 0:
        return None
    if n > MAX_DEVICES_PER_NODE:      # 노드당 디바이스 상한(위 주석)
        return None
    return n
...
#     게이트웨이가 "무엇에 무엇으로 답해야 하는가"의 정본. 서비스 계층이 이 표를
#     다시 만들면 표준 해석이 두 곳에 생긴다.
# ═══════════════════════════════════════════════════════════════
...
def expected_reply(kind: MsgKind | None) -> MsgKind | None:
    ...
    대기 요청의 매칭 기준이다. `Node ID` + `Message Identifier` 만 보면 다른
    요청의 지연·중복 Response 나 우연히 번호가 같은 ACK 가 현재 호출의 결과로
    반환된다. 0943 7.2.2 의 매칭은 '어느 요청의 응답인가'를 가리는 것이므로
