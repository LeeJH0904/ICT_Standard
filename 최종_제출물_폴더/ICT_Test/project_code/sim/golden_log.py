...
실물 보드 통합 단계의 실측 캡처가 준비되기 전까지 `logs/`는 골든 벡터만 재생한다
— 이 스크립트가 그 세션 로그를 만든다. 포맷은 `{"t","dir","hex"}`.
...
#: 골든 벡터 dir("N->G"/"G->N") -> 세션 로그 dir("rx"/"tx"). 게이트웨이 관점:
#: 노드가 보낸 것(N->G)이 게이트웨이의 수신(rx), 게이트웨이가 보낸 것(G->N)이
#: 게이트웨이의 송신(tx).
