...
#: 기능 2가 판정하는 위반 케이스 8종. 이 집합 밖의 ID는
#: 거부한다( 근거와 동일 — 임의 바이트열 주입 경로를 만들지 않는다).
