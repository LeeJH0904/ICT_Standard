...
`sim/virtual_node.py`가 노드 3종을 흉내낸다.
...
(그래야 독립 구현과의 교차검증이 의미가 있다), 그 결과에
이르는 절차는 다르다.

`sim/` 전용이다 — `siap/`·`backend/`는 이 파일을 참조하지 않는다
(계층 규칙과 무관하게, 애초에 `siap/codec.py`가 정본이고
이 파일은 정본을 흉내 내는 테스트 더블이다).

인코딩 규칙은 big-endian(network byte
...
MT_NOTI_DEVICE_VALUE = 0x0800        # 표 7-4 (NOTI_ERROR와 코드 중복)
...
class WireDP:
    ...
    period: int              # 14bit — sec (표준 미규정 결정과 같은 단위 결정)
...
def encode_dp(d: WireDP) -> bytes:
    ...
    구현이다(독립성 원칙 — 인코더 버그 상쇄 방지, 이 파일 머리말 참고)."""
    w = _Writer()
