...
찍고 종료하는 기본 동작은 그대로 유지한다. `--serve`를
주면 `backend.api.create_app()`으로 앱을 만들고 `uvicorn.run()`을 불러
Ctrl-C로 멈출 때까지 떠 있는다 — `web/`가 실제로 두드릴 서버가
이 경로에서 나온다. `simulate` 모드는 `sim/inject.py`를 감싼 `inject_fn`
도 함께 준다(`POST /api/v1/sim/inject`) — `virtual_node.py`의
로컬 제어 채널로 골든 벡터 원본 바이트를 흘려보낸다.
`on_frame`은 부수효과(DB 반영) 전용이다. 회신은 `on_frame`
이 설정되어도 여전히 `_default_reply()`가 만든다(표준 해석은 프로토콜
계층에만).
`on_frame`에 `backend.ingest.bind(conn)`을 직접 꽂지 않는다.
`bind(conn)`은 **호출자가 넘긴 연결을 그대로 클로저에 가둘 뿐**이라, 그
연결을 만든 스레드가 아닌 곳(SIAP I/O 스레드)에서 호출되면 `sqlite3.
ProgrammingError`로 죽는다(스레드별 연결 원칙,
`check_same_thread=False` 금지 — 재현 실측). 이 파일의
...
# 검증기·진입점 콘솔 출력은 CP949 표현 가능 범위
# 밖의 문자(em dash 등)에도 죽지 않아야 한다. 다른 검증기와
# 같은 가드.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="strict",
        help="strict(기본) / extended — 골든 벡터 분리와 대응",
    )
    parser.add_argument(
...
def _prepare_db_path(args: argparse.Namespace) -> Path:
    ...
    ("스레드별 연결", `check_same_thread=False` 금지),
    메인 스레드에서 연 연결을 I/O 스레드로 넘기면 첫 프레임 처리에서
    `ProgrammingError: SQLite objects created in a thread can only be
    used in that same thread`로 죽는다( 재현 실측)."""
...
def _make_on_frame(db_path: Path):
    """`on_frame`은 SIAP I/O 스레드 안에서 호출된다(`link._io_loop`).
...
def _make_inject_fn(control_port: int):
    """`POST /api/v1/sim/inject`가 부를 콜백. `simulate`
...
def _cp949_safe(s: str) -> str:
    ...
    다 — 이 함수는 호출부에 "이 출력은 콘솔 인코딩을 의식했다"는
    표식을 남기는 항등 함수다."""
    return s


class _JsonlCapture:
    """rx/tx 프레임을 logs/*.jsonl 포맷({"t","dir","hex"})으로 기록한다.
    SIAP I/O 스레드가 단독으로 부른다(link._io_loop) — 시리얼·소켓
    소유 스레드와 같아 별도 락이 필요 없다. t 는 첫 프레임을
    0.0 으로 한 상대 시각(초) — session_00_golden.jsonl 과 같은 관례라 replay 가
    간격을 정규화해 그대로 재생한다. tx 는 게이트웨이 응답이므로 replay 가
    바이트열을 대조하고, rx 는 노드가 보낸 것이라 주입된다."""
...
def _run_replay(args: argparse.Namespace) -> int:
    ...
    if args.serve:
        # 재생은 백그라운드에서 계속 진행되고(Replayer 는 자체
    ...
    # 무한 대기하지 않는다("좀비 스레드를 남기지 않는다"와 같은 원칙).
...
def _run_hardware(args: argparse.Namespace) -> int:
    ...
    try:
        if args.serve:
            from siap.build import FrameBuilderImpl
            builder = FrameBuilderImpl(1, args.proto, registry=link)
            # hardware 모드는 실물 링크에 조작된 프레임을 흘리지
            # 않는다 — inject_fn 을 아예 주지 않는다.
