...
§9.2 의 5단계 중 "2. DB 준비"는 원래 단계 5(`backend/`)가 채우기로
...
"5. uvicorn 기동"이 단계 6(`services/`·`api.py`) 완성 뒤에도
...
# CLAUDE.md §3.5 검증기·진입점 콘솔 출력은 CP949 표현 가능 범위
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--db", default=None,
                         help="DB 파일 경로 (기본: project_code/backend/runtime.db). "
                              "§9.2 '2. DB 준비': 없으면 schema.sql+seed 로 새로 만들고, 있으면 그대로 연다")
    parser.add_argument("--serve", action="store_true",
                         help="§9.2 '5. uvicorn 기동'. 주면 REST API를 띄우고 Ctrl-C "
...
def _prepare_db_path(args: argparse.Namespace) -> Path:
    """아키텍처 설계서 §9.2 '2. DB 준비'. 매 실행마다 호출한다
...
def _serve_app(*, db_path: Path, link, builder, run_mode: str, proto: str,
    ...
    """§9.2 '5. uvicorn 기동'. `create_app()`으로 앱을 만들고
