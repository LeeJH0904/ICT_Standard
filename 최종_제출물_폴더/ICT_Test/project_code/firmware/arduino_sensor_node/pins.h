...
/*
 * Uno 센서 노드 핀 매핑 — 보드 디렉터리 전용. core/ 는 이 파일을 모른다.
 *
 * 이 헤더는 Arduino 스케치(.ino)에서만 include 한다 — A0 류 상수는 Arduino.h 가
 * 정의한다. 순수 C 인 sensors.c 는 이 파일을 include 하지 않는다.
 */
 ...
 * 센서 종류는 보드 계층의 몫이며 core/ 는 바뀌지 않는다. */
