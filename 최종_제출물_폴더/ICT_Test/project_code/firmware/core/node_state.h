...
/*
 * 노드 상태 머신 — 0943 8장. siap_frame.c(스트리밍 코덱) 위에 8상태 전이 ·
 * pending 재전송 · due 회전 커서 · RES_SET_CONNECTION 오류 RSC 분류를 얹는다.
 *
 * core/ 는 하드웨어 의존성이 없다. 이 파일이 아는 하드웨어는 siap_io_t/
 * siap_dev_ops_t(siap_types.h) 뒤의 함수 포인터뿐이며, "온실 관제" · "온도
 * 센서" 같은 도메인 어휘는 등장하지 않는다 — 그래서 보드 3종이 공유한다.
 */
...
/* ═══════════════════════════════════════════════════════════════
 *  0. 노드 상태. 노드 자신이 보는 상태이며, 게이트웨이가 관리하는 세션
 *     상태와는 다른 열거형이다.
 * ═══════════════════════════════════════════════════════════════ */
typedef enum {
...
typedef struct {
    ...
    uint16_t msg_id;   /* 재전송에도 유지한다(새로 발번하면 노드가 중복 요청으로 처리) */
...
/* ═══════════════════════════════════════════════════════════════
 *  2. due 비트 + 회전 커서 — 주기 송신 3소스의 기아 방지.
 * ═══════════════════════════════════════════════════════════════ */
 ...
 *  RES_GET_DEVICE_VALUE)은 TX_WINDOW(51B)가 헤더+요소 하나분이라 청크 단위로
 *  나눠 보낸다. siap_io_t.write 는 부분 쓰기를 허용하는 논블로킹 계약이라, 이
 *  구조체가 "지금 몇 번째 청크까지 나갔는가"를 들고 있어 poll() 이 이어서 마저
 *  보낼 수 있게 한다. */
typedef enum {
    SIAP_SEQ_NONE = 0,
    ...
    SIAP_SEQ_REQ_SET_NODE_DEVICE_PROPERTY_ALL,  /* 노드→GCG 디바이스 구성 선언(0943 8.1.3.3).
...
typedef struct {
    ...
    uint8_t  sw_version;
    const siap_io_t     *io;      /* 전송 계층(UART/TCP) 콜백 */
    const siap_dev_ops_t *dev_ops;/* 디바이스 read/write 콜백 */
...
/* 표 7-18 기본값 — Message Receive Timeout 2s · Num. of Retry 3회 · Notify Error
   Interval 30s · Keep Alive Interval 60s. 전부 sec 단위다. */
...
typedef struct {
    ...
    uint16_t next_msg_id;      /* 0 부터(0943 7.2.2). 0도 유효한 순번, 0xFFFF 다음 0 */
    siap_pending_t pending;    /* 동시 대기 1건. NOTI_DEVICE_VALUE 는 arg 에
                                   보낼 devices[] 인덱스 비트마스크를 담는다 */
    siap_tx_seq_t  tx_seq;     /* 다중 청크 송신 진행 상태 */

    /* 주기 송신 3소스 공통 회전. t_keep_alive/t_error 는 다음 만료 절대 시각(ms).
       DEVICE_VALUE 는 디바이스별로 갈린다 — 아래 dev_* 참조. */
    ...
    /* 디바이스별 스캔 스케줄. Period(표 7-15)의 표준상 의미는 "데이터 전달주기"다 —
       본 구현에 샘플링 주기 전용 필드가 없어 이를 내부 스캔 간격으로도 재사용할
       뿐이며, 표준 필드 의미의 재정의가 아니다(0943 미규정 결정). 스캔 결과의
       전송 여부만 Transfer Mode 가 가른다. dev_due 의 비트 i 는 devices[i]가 스캔
       대상이 됐고 아직 처리 전임을 뜻한다 — 처리가 pending 적체로 미뤄져도 잃지 않는다. */
    uint32_t dev_next_due[SIAP_MAX_DEVICES_PER_NODE];
    uint16_t dev_due;

    /* FAULT(0943 8.2.1.1) — 결함은 노드 전체에 대해 하나만 추적한다(NOTI_ERROR 는
       표 7-4 상 device_id 필드가 없다). */
    ...
    /* REBOOTING → BOOT 완료 후 실제 재기동은 보드의 몫이다(core/ 는 재기동
       "수단"을 모른다). 이 콜백이 NULL 이 아니면 BOOT 전이 시 한 번 호출한다 —
       호스트 테스트는 NULL 로 두고 state==SIAP_NS_BOOT 도달만 관찰한다. */
...
/* 초기화 — 0943 진입점 범위 검증을 강제한다: gcg_id/node_id 는 0~2^20-1,
   device_count 는 1~16, device_id 는 노드 내 유일, subtype 은 레지스트리 등재값,
   value_type 은 0~2, period 는 0~2^14-1. 하나라도 벗어나면 false(노드가 뜨지 않는다). */
bool siap_node_init(siap_node_t *node, const siap_node_cfg_t *cfg);

/* 논블로킹 한 틱. 보드 루프가 계속 부른다. 매 호출마다 가능한 만큼 바이트를 읽고
   먹이고, pending timeout 을 검사하고, RUNNING 에서는 due 소스를 회전 송신한다. */
