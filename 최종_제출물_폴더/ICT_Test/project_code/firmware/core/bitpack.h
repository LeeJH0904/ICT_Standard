 ...
 * 플랫폼 헤더를 include하지 않는다. <stdint.h> ·
 * <stddef.h> · <stdbool.h> 는 C99 표준 헤더이지 플랫폼 헤더가 아니다.
 * 호스트 테스트가 이를 기계적으로 확인한다.
 */
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
/* C++/Arduino 스케치에서 C 링키지로 부를 수 있게 한다 — C 컴파일 시엔 비활성.
   __cplusplus 는 보드 판별 매크로가 아니라 언어 매크로라 core 순수성·
...
/* GCC · Clang · AVR-GCC 공통. 미지원 컴파일러에서는 빈 매크로가 되지만
   호스트 테스트가 GCC/Clang 으로 도는 한 누락은
   거기서 잡힌다. */
#if defined(__GNUC__)
 ...
 *   바이트 순서: big-endian (표준 미규정 → 자체 결정).
 ...
 * siap_frame.c 의 몫이다). nbits 가 0 이거나 32 를
