 ...
 * on_element/on_end) 위에서 8상태 전이 · pending 재전송 · due 회전 커서를
 * 구현한다. 이 파일은 하드웨어를 모른다 — 아는 것은 siap_io_t/siap_dev_ops_t
 * 뒤의 함수 포인터뿐이다.
 *
 * REQ_SET_DEVICE_CONTROL 수신 → dev_ops.write_value 가 구동기를 움직일 수 있는
 * "유일한 경로"다. 이 파일에서 write_value 를 호출하는 곳은 _on_element() 의
 * SIAP_REQ_SET_DEVICE_CONTROL 분기 한 군데뿐이다(사람 검토 지점의 하드 경계).
 */
#include "node_state.h"

/* 프레임 간 무입력을 "침묵"으로 보는 기준. 9600 baud 에서 1 byte = 1.04 ms
   이므로 20 byte 분이다(재동기 T_gap). */
#define SIAP_T_GAP_MS 20u

/* 표 7-18 기본값. 시간 3필드는 전부 sec. Num. of Retry 기본 3회 — 재전송이
   전부 실패해야 미수집으로 본다. */
...
/* 표 7-8 헤더 필드로부터 "이 프레임이 나를 향한 것인가"를 판정한다. 유니캐스트는
   Node ID(+GCG ID) 일치를 요구하고, 멀티/브로드캐스트는 전부 받는다(표 7-6). */
...
/* 0943 7.2.2 — "Message Identifier는 … '0'에서 '65535'까지 사용할 수 있다.
   일련번호는 데이터 전송 시마다 +1을 하며 만료되면 0부터 다시 시작한다."
   pending 슬롯의 "비어 있음"은 pending.kind==SIAP_KIND_NONE 으로만 판정하므로
   msg_id==0 도 유효한 순번으로 그대로 쓴다. uint16_t 덧셈은 0xFFFF+1 을 0 으로
   감아 표준 문구와 그대로 맞다. */
...
/* RES_SET_CONNECTION 오류 RSC 분류 — 재시도 가능 2종(INVALID_GCG_ID·
   INVALID_NODE_ID) / 불가 7종. 불가는 HALTED 로 간다. */
static bool _rsc_retryable(siap_rsc_t rsc)
{
    return rsc == SIAP_RSC_INVALID_GCG_ID || rsc == SIAP_RSC_INVALID_NODE_ID;
}

/* DISCONNECTED 백오프 — Timeout × 2ⁿ, 상한 300 s. 진입마다 shift 를 늘리고
...
/* RES_* 빌더 구역에 정의된다 — 다중 청크 시퀀서가 헤더 청크를 만들 때 먼저
   필요해 전방 선언한다. */
static siap_np_t _build_np(const siap_node_t *node);

/* siap_tx_flush(siap_frame.h)가 요구하는 콜백 시그니처(size_t 반환)와
   siap_io_t.write(int16_t 반환)는 폭이 다르다 — 어느 쪽도 고치지 않고 얇은
   어댑터로 잇는다. ctx 자리에 siap_io_t* 를 그대로 넘긴다. */
...
/* 인코더 윈도우를 flush 한다. 부분 쓰기면 다음 poll 이 이어서 flush 한다.
   동시 대기 1건 원칙 위에서 새 프레임을 큐잉하기 전에는 항상 이전 프레임이 이미
   다 나갔거나(HALTED·ACK 처럼 즉시 송신) 큐를 새로 쓰는 것이 안전하다는 전제를
   둔다 — 호스트 테스트의 write 는 항상 전량을 즉시 받아써 이 전제가 성립한다. */
 ...
 *  siap_tx_reset() 을 부르지 않는다 — _tx_seq_pump() 의 while 조건이 그
 *  불변식을 강제한다. 헤더/요소를 만드는 두 헬퍼는 "이번 청크를 win 에 쓴다"만
 *  하고, 실제로 내보내는 것과 다음 청크로 넘어갈지 판단하는 것은 pump 책임이다.
 * ═══════════════════════════════════════════════════════════════ */
...
static bool _tx_seq_build_header(siap_node_t *node)
{
    ...
    case SIAP_SEQ_REQ_SET_NODE_DEVICE_PROPERTY_ALL: {
        /* 노드→GCG 디바이스 구성 선언(0943 8.1.3.3). 요청이라 RSC 가 없다.
           payload = NODE_PROPERTY(8B) + DEVICE_PROPERTY(30B)×N. 요소(DP×N)는
...
static bool _tx_seq_build_element(siap_node_t *node, uint8_t dev_idx)
{
    ...
    if ((siap_tx_seq_kind_t)s->kind == SIAP_SEQ_NOTI_DEVICE_VALUE) {
        uint32_t raw;
        /* 재인코딩 시점의 현재값. 실패하면 마지막으로 알려진 값을 그대로 보낸다 —
           새 오류감지·FAULT 진입은 due_send_next 의 주기 스캔이 담당한다. */
...
/* 진행 중인 시퀀스를 가능한 만큼 밀어낸다. 청크 하나가 완전히 flush될 때만
   (tx_busy==false) 다음 청크를 만든다 — 부분 쓰기(UART 포화)면 while 조건에서
   멈추고 다음 poll() 이 이어받는다. */
...
static bool _pending_encode(siap_node_t *node)
{
    ...
    if (kind == SIAP_REQ_SET_NODE_DEVICE_PROPERTY_ALL) {
        /* 다중 청크 요청. "ALL"이므로 전체 디바이스를 선언한다. 재전송도 같은
           msg_id 로 현재 devices[] 구성을 그대로 다시 편다("재전송은 재인코딩"). */
        siap_tx_seq_t *s = &node->tx_seq;
...
/* Timeout 만료 시 재전송 또는(소진 시) 상태별 사건. */
...
/* 만료 시각을 interval 만큼 미룬다 — 딱 한 번만 더하면, 이번 만료가 이미 한 주기
   이상 늦게 감지된 경우(poll 이 뜸하게 불렸다) 새 만료 시각이 여전히 now 이하로
   남아 바로 다음 호출에서 또 만료로 잡힌다. now 를 넘어설 때까지만 더한다 —
   "현재 시각으로 리셋"과는 다르다: 리셋은 매번 now 를 기준으로 삼지만, 이건 원래
   스케줄에 interval 의 정수배만 더해 따라잡는다. */
...
/* Value(USER DEPENDENT 타입은 main.value_type 을 따른다)가 [Lower Value,
   Upper Value] 밖인가 — 표 7-15 Event/Both 전송 조건. 타입별 원시 비트열 해석은
   siap_frame.h 의 siap_value_as_*() 로 통일한다. */
...
/* 디바이스별 스캔 스케줄. Period(표 7-15)의 표준상 의미는 "데이터 전달주기"다 —
   이 재사용은 그 의미를 재정의하는 것이 아니다. 본 구현에 샘플링 주기 전용
   필드가 없어 Period 를 내부 스캔 간격으로도 재사용할 뿐이다(0943 미규정 결정).
   Periodic/Both/Event 전부 자기 Period 로 스캔되고, 스캔 *결과의 처리* 만
   Transfer Mode 가 가른다(Both 는 이 구현에서 Periodic 과 동일하게 동작한다).
   Event-only 노드도 이 스캔으로 오류를 감지한다(0943 8.2.1.1). */
...
/* pending 이 비어 있을 때만 불린다(동시 대기 1건). due 소스를 cursor 순서로
   최대 3번 훑어 이번 poll 에서 보낼 하나를 고른다. */
static void _due_send_next(siap_node_t *node, uint32_t now)
{
    for (uint8_t tries = 0; tries < 3u; tries++) {
        ...
        if (bit == SIAP_DUE_DEVICE_VALUE) {
            ...
            /* 이번 스캔에서 due 로 표시된 디바이스를 순서대로 처리한다. 읽기 실패
               첫 건에서 즉시 FAULT 로 멈춘다(0943 8.2.1.1, 결함은 노드당 하나만
               추적) — 그 시점까지 성공한 디바이스는 값·Status 만 갱신되고 이번
               회차엔 전송되지 않는다(다음 스캔에 다시 due 로 잡힌다 — 데이터
               유실이 아니라 한 주기 지연이다). */
            ...
            for (uint8_t i = 0; i < node->cfg.device_count; i++) {
                ...
                if (node->cfg.dev_ops->read_value(node->cfg.dev_ops->ctx, d->main.device_id, &raw) != 0) {
                    ...
                    node->fault_nec = (uint8_t)SIAP_NEC_ERROR_DEVICE_INTERFACE;
                    faulted = true;
                    break;
                }
                d->main.value = raw;
        ...
        if (bit == SIAP_DUE_ERROR) {
            ...
            node->due = (uint8_t)(node->due & (uint8_t)~SIAP_DUE_ERROR);
            {
                ...
                if (node->cfg.dev_ops->read_value(node->cfg.dev_ops->ctx, fd->main.device_id, &raw) == 0) {
                    ...
                    node->state = SIAP_NS_RUNNING;   /* 오류 해소 */
                    return;
                }
            }
            _pending_begin(node, SIAP_NOTI_ERROR, node->fault_nec, now);
...
/* REQ_GET_DEVICE_PROPERTY/REQ_GET_DEVICE_VALUE 가 rx_ids[] 에 모아둔 device_id
   들 중 실제 등록된 것만 골라 idx[] 에 채운다. */
...
static void _reply_get_device_property(siap_node_t *node)
{
    ...
    _tx_seq_pump(node);   /* TX_WINDOW(51B)는 요소 하나분 — 청크마다 완전히 flush 후 진행 */
}
...
static int8_t _on_header(void *ctx, const siap_hdr_t *h, siap_kind_t k, uint16_t n)
{
    ...
    node->rx_ids_n      = 0;
    /* 라우팅·상태 게이팅은 on_fixed/on_element/on_end 에서 node->state 를 직접
       보고 결정한다 — 여기서는 항상 수락해 스트리밍 파싱 자체는 계속 진행시킨다. */
    return 0;
}
...
static int8_t _on_fixed(void *ctx, const uint8_t *buf, uint8_t len)
{
    ...
    if (node->rx_kind == SIAP_RES_SET_NODE_DEVICE_PROPERTY_ALL) {
        /* 노드가 보낸 디바이스 구성 선언의 응답. 고정부 RSC 를 꺼내 on_end 로
           전파한다(RES_SET_CONNECTION 과 같은 관례 — 성공이면 0, 실패면 -(rsc)).
           응답 매칭: pending 종류·msg_id 가 맞아야 한다. */
...
static void _on_end(void *ctx, siap_rsc_t rsc, siap_clause_t clause)
{
    ...
    case SIAP_RES_SET_CONNECTION:
        ...
        if (rsc == SIAP_RSC_SUCCESS) {
            ...
            /* REQ_SET_CONNECTION(0943 8.1.1)은 페이로드가 없어(LAYOUT (0,0))
               디바이스 구성을 실을 수 없다. 연결 성공 직후 이 세션의 전체 구성을
               REQ_SET_NODE_DEVICE_PROPERTY_ALL 1회로 선언한다(0943 8.1.3.3). pending
               에 실어 RES_SET_NODE_DEVICE_PROPERTY_ALL 수신까지 재전송 타이머가
               재시도한다 — 그 응답 전에는 pending 이 차 있어 NOTI_DEVICE_VALUE
               텔레메트리가 뒤로 밀린다(선언 우선). */
            _pending_begin(node, SIAP_REQ_SET_NODE_DEVICE_PROPERTY_ALL, 0, now);
        } else if (_rsc_retryable(rsc)) {
            /* 응답을 받았어도 즉시 재송신하지 않는다 — pending 을 그대로 두어 기존
               재전송 타이머가 Timeout 뒤에 같은 msg_id 로 재시도하게 한다. */
        } else {
            _pending_clear(node);
    ...
    case SIAP_ACK: {
        /* 응답 매칭은 Node ID + Message Identifier + Message Type 셋 다 맞아야
           한다. msg_id 만 같다고 아무 pending 이나 해제하면 안 된다: pending 이
           REQ_SET_CONNECTION(응답은 ACK 가 아니라 RES_SET_CONNECTION)일 때 msg_id
           가 우연히 같은 ACK 가 와도 무시해야 한다 — 그렇지 않으면 pending 이
           사라진 채 CONNECTING 에 남아 재전송도 못 하고 영구 정지한다. ACK 를
           기대하는 pending 은 Notify 4종뿐이다. */
    ...
    case SIAP_NOTI_DISCONNECT:
        ...
        _send_ack_now(node, &node->rx_hdr);   /* 상태 무관 즉시 ACK */
        _pending_clear(node);
        node->due = 0;
        node->dev_due = 0;
        node->state = SIAP_NS_DISCONNECTED;
    ...
    case SIAP_NOTI_REBOOT:
        if (rsc != SIAP_RSC_SUCCESS) return;
        /* 0943 6.1.2/8.2.1.4 — 게이트웨이발 리부팅 알림도 정상 수신하면 상태
           게이트보다 먼저 ACK 한다. 수신 알림은 노드 자신의 리부팅 명령이 아니므로
           현재 상태와 송신 pending 은 유지한다. */
        _send_ack_now(node, &node->rx_hdr);
        return;

    /* ── 이하 RUNNING 전용 — "G→N 요청 수신 → RES_* 회신" ── */
    case SIAP_REQ_SET_DEVICE_CONTROL:
    ...
    case SIAP_RES_SET_NODE_DEVICE_PROPERTY_ALL:
        /* 노드가 보낸 디바이스 구성 선언(REQ_SET_NODE_DEVICE_PROPERTY_ALL)의 응답.
           응답 매칭(Node ID·Message Identifier·Message Type)이 맞고 SUCCESS 면
           pending 을 해제해 재전송을 멈춘다. 실패면(rsc 는 on_fixed 가 전파)
           pending 을 두어 재전송 타이머가 재시도하게 한다. */
        if (node->pending.kind != (uint8_t)SIAP_REQ_SET_NODE_DEVICE_PROPERTY_ALL
            || node->rx_hdr.msg_id != node->pending.msg_id)
            return;
        if (rsc == SIAP_RSC_SUCCESS)
...
{
    /* 자가진단 실패는 연결을 막지 않는다. 실패분만 ABNORMAL 로 두고 계속
       진행한다(실패 시 값을 지어내지 않는다). */
    for (uint8_t i = 0; i < node->cfg.device_count; i++) {
        siap_dp_t *d = &node->cfg.devices[i];
        uint32_t raw;
        if (node->cfg.dev_ops->read_value(node->cfg.dev_ops->ctx, d->main.device_id, &raw) == 0) {
...
bool siap_node_init(siap_node_t *node, const siap_node_cfg_t *cfg)
{
    ...
    node->next_msg_id = 0;   /* 0943 7.2.2 — 0 도 유효한 순번이다 */
    node->pending.kind = (uint8_t)SIAP_KIND_NONE;
...
void siap_node_poll(siap_node_t *node)
{
    ...
    if (node->state == SIAP_NS_HALTED) {
        /* 수신 프레임을 읽어 버리고 응답하지 않는다. 전원 재인가로만 벗어난다
           (상태 무관 전이의 유일한 예외). */
        uint8_t b;
    ...
    /* 수신 — 이번 poll 에서 가능한 만큼 바이트를 먹인다. */
    {
        ...
        for (;;) {
            ...
            if (rb < 0) {
                /* 링크 오류 — NOTI_DISCONNECT 수신과 같은 취급 */
    ...
    /* 이전에 못 다 내보낸 프레임 이어서 flush */
    if (node->tx_busy) _tx_flush(node);
    /* 청크 하나가 방금 완전히 나갔으면 다중 청크 시퀀스의 다음 청크로 이어간다 —
       tx_busy 인 동안은 절대 부르지 않는다. */
