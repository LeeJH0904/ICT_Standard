 ...
 * on_element/on_end) 위에서 8상태 전이 · pending 재전송 ·
 * due 회전 커서를 구현한다. 이 파일은 하드웨어를 모른다 — 아는
 * 것은 siap_io_t/siap_dev_ops_t 뒤의 함수 포인터뿐이다(소유 경계).
 *
 * — REQ_SET_DEVICE_CONTROL 수신 → dev_ops.write_value 가 구동기를
 * 움직일 수 있는 "유일한 경로"다. 이 파일 안에서
 * write_value 를 호출하는 곳은 _on_element() 의 SIAP_REQ_SET_DEVICE_CONTROL
 * 분기 한 군데뿐이다.
 */
#include "node_state.h"

/* 프레임 간 무입력을 "침묵"으로 보는 기준. 9600 baud에서 1 byte
   = 1.04 ms 이므로 20 byte 분이다. */
#define SIAP_T_GAP_MS 20u

/* 표 7-18 기본값. 시간 3필드는 전부 sec. Num. of Retry 기본 3회이며,
   재전송이 전부 실패해야 미수집으로 본다. */
...
/* — 재시도 가능 2종(INVALID_GCG_ID·INVALID_NODE_ID) / 불가 7종.
   · 표와 동일하다. */
...
/* Timeout × 2ⁿ, 상한 300 s. DISCONNECTED 진입마다 shift 를 늘리고
...
/*. RES_* 빌더 구역에 정의된다 — 다중 청크 시퀀서가 헤더 청크를
   만들 때 먼저 필요해 전방 선언한다. */
static siap_np_t _build_np(const siap_node_t *node);

/* siap_tx_flush(siap_frame.h)이 요구하는 콜백 시그니처(size_t 반환)와
   siap_io_t.write(int16_t 반환)는 폭이 다르다 — 둘
   다 이미 확정된 인터페이스라 어느 쪽도 고치지 않고 얇은 어댑터로
   잇는다. ctx 자리에
   siap_io_t* 를 그대로 넘긴다. */
...
. 동시 대기 1건 원칙 위에서 새 프레임을 큐잉하기 전에는
...
/* Timeout 만료 시 재전송 또는(소진 시) 상태별 사건(표). */
...
/* 만료 시각을 interval 만큼 미룬다 — 딱 한 번만 더하면, 이번 만료가 이미
   ...
   보낸다"는  전제가 깨진다). now 를 넘어설 때까지만 더한다 —
   "현재 시각으로 리셋"(금지)과는 다르다: 리셋은 매번 now 를 기준으로
...
/* pending 이 비어 있을 때만 불린다(동시 대기 1건). due 소스를
        ...
        if (bit == SIAP_DUE_DEVICE_VALUE) {
            ...
            for (uint8_t i = 0; i < node->cfg.device_count; i++) {
                ...
                if (node->cfg.dev_ops->read_value(node->cfg.dev_ops->ctx, d->main.device_id, &raw) != 0) {
                    ...
                    node->fault_nec = (uint8_t)SIAP_NEC_ERROR_DEVICE_INTERFACE;   /* 인용문 근거 */
                    faulted = true;
                    break;
                }
                d->main.value = raw;
...
static int8_t _on_header(void *ctx, const siap_hdr_t *h, siap_kind_t k, uint16_t n)
{
    ...
    /* 라우팅·상태 게이팅은 on_fixed/on_element/on_end 에서 node->state 를
       직접 보고 결정한다("표에 없으면 무시") — 여기서는 항상 수락해
       스트리밍 파싱 자체는 계속 진행시킨다(요소 단위 즉시 적용). */
    return 0;
}
...
static void _on_end(void *ctx, siap_rsc_t rsc, siap_clause_t clause)
{
    ...
    case SIAP_RES_SET_CONNECTION:
        ...
        if (rsc == SIAP_RSC_SUCCESS) {
            ...
            /* REQ_SET_CONNECTION(8.1.1)은 페이로드가 없어(LAYOUT (0,0))
               ...
               에 실어 RES_SET_NODE_DEVICE_PROPERTY_ALL 수신까지 재전송
            ...
            /* — "Timeout 후 재송신, msg_id 유지". 응답을 받았어도
               즉시 재송신하지 않는다 — pending 을 그대로 두어 기존 재전송
               타이머가 Timeout 뒤에 같은 msg_id 로 재시도하게 한다. */
        } else {
            _pending_clear(node);
    ...
    case SIAP_RES_SET_NODE_DEVICE_PROPERTY_ALL:
        /* 노드가 보낸 디바이스 구성 선언(REQ_SET_NODE_DEVICE_PROPERTY_ALL)
           ...
           가 전파) pending 을 두어 타이머가 재시도하게 한다. */
        if (node->pending.kind != (uint8_t)SIAP_REQ_SET_NODE_DEVICE_PROPERTY_ALL
            || node->rx_hdr.msg_id != node->pending.msg_id)
            return;
        if (rsc == SIAP_RSC_SUCCESS)
...
{
    /* — 자가진단 실패는 연결을 막지 않는다. 실패분만 ABNORMAL 로 두고
...
void siap_node_poll(siap_node_t *node)
{
    ...
    /* 수신 — 이번 poll 에서 가능한 만큼 바이트를 먹인다. */
    {
        ...
        for (;;) {
            ...
            if (rb < 0) {
                /* 링크 오류 — NOTI_DISCONNECT 수신과 같은 취급(마지막 행) */
