...
#define SIAP_TYPES_H
/*
 * SIAP 프레임 계약의 C 정의 — TTAK.KO-10.0943 7장.
 *
 * 이 파일이 0943 7장의 메시지 종류·헤더·페이로드 구조·상태 코드를 C 타입으로
 * 옮긴 정본이다. Uno · Pro Mini · ESP32 가 모두 이 한 파일을 공유하므로
 * "동일 응용계층"이 성립한다(core/ 는 플랫폼 헤더를 include 하지 않는다).
 *
 * 0943 미규정 → 본 구현이 확정한 사항:
 ...
 *  0. 헤더 상수 — 그림 7-1
 * ═══════════════════════════════════════════════════════════════ */
#define SIAP_VERSION 0x12u   /* v1.2, 0943 7.2.1 */

/* ═══════════════════════════════════════════════════════════════
 *  1. 메시지 종류 (표 7-2 ~ 7-4).
 *     SIAP_KIND_NONE(0) 은 "해석 불가"를 나타내는 센티널이다.
 * ═══════════════════════════════════════════════════════════════ */
typedef enum {
    ...
    /* Notify / ACK 6종 (표 7-4). 표 7-4 는 NOTI_ERROR 와 NOTI_DEVICE_VALUE 에
       같은 코드 0x0800 을 준다 — WIRE_CODE 값만 겹칠 뿐 열거형 멤버는 고유하다. */
    SIAP_NOTI_ERROR,
...
/* 표 7-6. 미정의값(0x03)은 열거형 밖이라 raw uint8_t 로 다루고
   siap_trans_type_valid() 로만 판정한다. */
...
typedef enum {                    /* 표 7-10 (Response Status Code). 원문 표기는 'SUCESS'(오타) */
...
/* 0x0A~0xFF 는 Reserved(표 7-10). 열거형 밖 raw 값이 올 수 있어 별도 판정한다. */
static inline bool siap_rsc_valid(uint8_t v) { return v <= (uint8_t)SIAP_RSC_INVALID_FORMAT; }

typedef enum {                    /* 표 7-12 (Notify Error Code) */
...
/* 표 7-14 Value Type. RESERVED(0x03)는 열거형 밖이라 raw 로 담고 비교만 한다 —
   Value Type=0x03 수신은 INVALID_DATA_TYPE(0x06, 표 7-10)로 판정한다. */
...
/* Transfer Mode 0x03, Status 0x03~0xFF 는 Reserved(표 7-15, NODE_PROPERTY 는 표 7-13). */
...
/* ═══════════════════════════════════════════════════════════════
 *  3. 구조체 크기 상수 (0943 7.3.3) — 코덱이 참조하는 정본
 * ═══════════════════════════════════════════════════════════════ */
...
/* 노드당 디바이스 상한 — 0943 미규정, 본 구현이 16 으로 확정(AVR SRAM 예산과
   RX/TX 스트리밍 윈도우 크기의 근거). */
...
typedef struct {              /* 표 7-15 (DEVICE_PROPERTY) — 240 bit.
                                  Lower/Upper Value·Limit·Precision(USER DEPENDENT)은
                                  main.value_type 을 따른다(0943 미규정 → 구현 결정). */
    siap_dmi_t main;
...
typedef struct {              /* 표 7-18 (MSG_CONTROL_PROFILE) — 56 bit. 시간 3필드 전부 sec */
...
/* ═══════════════════════════════════════════════════════════════
 *  5. WIRE_CODE — 논리 종류 → 전송 코드(표 7-2~7-4).
 *     strict   = 표준 원문 그대로 (0x0800 중복 포함, 고유 코드 33개).
 *     extended = 0x0800 중복을 해소한 개정 제안안 (고유 코드 34개).
 * ═══════════════════════════════════════════════════════════════ */
static const uint16_t SIAP_WIRE_CODE[SIAP_KIND_COUNT] = {
    ...
    [SIAP_NOTI_DEVICE_VALUE]                = 0x0800,  /* ★ 표 7-4 원문 중복 */
 ...
 *     (전송 계층·센서 I/O 만 보드별로 다르고 프로토콜은 core/ 가 공통 처리).
 * ═══════════════════════════════════════════════════════════════ */
typedef struct {
...
/* ═══════════════════════════════════════════════════════════════
 *  6. LAYOUT — (고정부 byte, 요소 byte). 가변 요소 개수 N 산출의 정본.
 *     0943 은 N 전달 방법을 규정하지 않아, 본 구현은 Payload Length 에서
 *     역산한다: N = (payload_len - fixed) / elem.
 * ═══════════════════════════════════════════════════════════════ */
