...
없다 — 새 의존성을 추가하려면 "의존성 최소화, 추가 전 사용자
확인"을 거쳐야 한다. `pytest backend/tests/test_api.py`는
오프라인 판정 환경에서도 반드시 통과해야 하므로, 기존 자체 평가기와 같은 원칙으로
표준 라이브러리(`asyncio`)만으로 ASGI 요청 1건을 흘려보내는 최소
드라이버를 둔다. 여러 검증기도 이 모듈을 공유한다 — 같은 이유로
같은 반례가 생기지 않게 한다. 검증기가 검증 대상과 다른 독립 경로로
요청을 흘려보내면 자기 검증
