"""backend/tests/test_db.py — 연결 팩토리."""
...
def test_foreign_keys_on_every_connection(tmp_path):
    """SQLite 기본값은 OFF다 — db.py 가 매 연결마다 켜야 한다."""
    path = tmp_path / "fk.db"
...
def test_seed_owned_tables_stay_empty_without_runtime_writer(fresh_db):
    """① — `device_manage`는 REQ_SET_CONNECTION 런타임
...
def test_seed_loads_control_model_and_public_data_source(fresh_db):
    """`control_model`·`public_data_source`는 등록 API가 없으므로
    `fixtures/seed.sql`이 유일한 등록 수단이다. 시드 이후로는
    ...
    수집 이력만 `public_data_record`에 API 스레드가 쓴다."""
