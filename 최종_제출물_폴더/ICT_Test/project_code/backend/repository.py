...
식별자는 UUID4(TEXT), 시간은 ISO 8601(TEXT) — 1369-P1 6.1.
"""
...
def record_config_change(conn: sqlite3.Connection, *, table_name: str, row_id: str, operation: str,
    ...
    """1369-P1 6.2.1 "설정형 데이터는... 변경(생성, 수정, 삭제)에 대해 이력이
    관리되어야 한다". 설정형 데이터 변경은 런타임의 `REQ_SET_DEVICE_PROPERTY`·
    `REQ_SET_NODE_DEVICE_PROPERTY_ALL` 디바이스 선언(노드발)과 사용자발 설정 변경
    양쪽에서 일어난다. `user_id`는 nullable — 노드발 Plug & Play 경로는 사람이
    유발하지 않아 None이고, 사용자발 설정은 서비스가 API 스레드에서 실제 사용자
    ID를 넘긴다. `version`은 DEFAULT 1을 그대로 쓴다(표준 미규정)."""
    id_ = new_id()
...
def get_or_create_device_info(conn: sqlite3.Connection, *, device_kind: str,
    ...
    `device_characteristics`는 `manufacturer`와 같은 자격이다: 0943 DEVICE_PROPERTY
    (표 7-15)가 나르지 않는 속성이라 동적 등록 경로는 채우지 않고 항상 `None`을
    넘긴다 — 6.2.4가 요구하는 컬럼 자체는 존재해야 하므로 저장 수단은 열어 둔다.

    실제로 새 행을 만들 때만 `config_change_log`에 CREATE 1건을 남긴다."""
...
def upsert_device_install_info(conn: sqlite3.Connection, *, device_info_id: str, device_name: str,
    ...
    `installed_at`(6.2.5 설치일자)도 최초 설치 시점의 사실이므로 재연결 UPDATE
    에서는 건드리지 않는다(의미상 `created_at`과 같은 부류). 이 참조 구현에서
    "설치"는 곧 "첫 디바이스 속성 선언 등록"이다.

    재연결로 장치 종류(subtype)가 바뀌면 `device_info_id`도 함께 갱신한다 —
    그래야 설치 행이 가리키는 `device_info.device_kind`가 실제 종류와 맞는다
    (1369-P1 7.1(6) "장치 설치 정보는 정확히 하나의 장치 기본 정보를 가진다",
    7.2.2.5 "device_info_id는 갱신 가능").

    `install_location`·`install_loc_unit`·`unit`은 `COALESCE`로 "호출자가 실제
    값을 줬을 때만 갱신"한다 — 한 번 설정된 값이 정보 없는 재연결에 의해 사라지지
    않게 한다. 그래서 온실 위치 기본값은 **최초 등록(CREATE)에서만** 넘기고
    재연결(UPDATE)에서는 `None`을 넘긴다.
...
def link_device_manage(conn: sqlite3.Connection, user_id: str, install_id: str) -> None:
    ...
    있으면 조용히 건너뛴다(재연결 시 재삽입 금지 — `link_device_install()` 과 같은
    멱등 패턴)."""
    exists = conn.execute(
...
def get_greenhouse_location(conn: sqlite3.Connection, greenhouse_id: str) -> tuple[str | None, str | None]:
    ...
    없다(0943 REQ_SET_CONNECTION은 위치를 나르지 않는다) — 장치 관리자를 소속
    온실 관리자로 삼는 것과 같은 근거로, 그 장치가 설치된 온실 자신의 위치
    (`greenhouse_info.location`/`location_unit`)를 기본값으로 쓴다. 온실 위치도
...
def get_default_greenhouse_id(conn: sqlite3.Connection) -> str | None:
    """이 참조 구현은 온실 1개 고정 데모다(`fixtures/seed.sql`). 온실이 없으면
    None — 호출자가 그 프레임 처리를 건너뛴다.
...
def record_env_measurement(conn: sqlite3.Connection, *, install_id: str, greenhouse_id: str,
    ...
    # 여기서 미리 비워 스키마의 이 결정을 그대로 존중한다(1369-P1 6.3.3.8).
...
def record_device_state(conn: sqlite3.Connection, *, install_id: str, subtype: str,
    ...
    if subtype == "WINDOW_OPENER":
        ...
        # power 하나만 주 필드로 채운다(FAN과 같은 패턴) — 관측 근거가 하나이므로
        # 위 결정("주 필드에만 값을 싣고 나머지는 NULL")을 그대로 지킨다.
...
#  CONTROL_TIMEOUT/NO_DATA(API 스레드)가 함께 쓴다. `SiapLink.send()`가 동기
#  반환이라 재전송 소진을 API 스레드가 그 자리에서 알고, 미수집 판정도 조회
#  시점에 API 스레드에서 돈다(전용 스케줄러 스레드를 새로 두지 않는다).
# ═══════════════════════════════════════════════════════════════
...
def record_alert(conn: sqlite3.Connection, *, kind: str, severity: str, message: str,
    ...
    """CHECK(siap_nec → frame_id) — `siap_nec`가 있으면 `frame_id`도 반드시 있어야
    한다(0943 8.2.1.1). 호출자는 NEC 알림 기록 시 항상 `frame_id`를 함께 넘긴다."""
...
def list_alerts_page(conn: sqlite3.Connection, *, since: str | None = None, until: str | None = None,
    ...
    """`GET /api/v1/alerts`(0937 6.4-3 · 6.5-2) 전용. `list_alerts()`는 `list[Alert]`
    계약이 굳어 있어 시그니처를 바꾸지 않고, 페이지네이션(`total` 포함)이 필요한
    이 호출을 별도 함수로 둔다."""
    where: list[str] = []
    params: list = []
...
# ═══════════════════════════════════════════════════════════════
#  API 스레드가 쓰는 테이블과 읽기 전용 조회. 이 절 아래는 backend/services/*.py ·
#  backend/api.py 가 부르는 SQL이다. 쓰기 소유권은 호출자(API 스레드)가 지킨다.
# ═══════════════════════════════════════════════════════════════
...
def list_public_data_sources(conn: sqlite3.Connection) -> list[models.PublicDataSource]:
    """`public_data_source`는 시드 전용 — 여기선 조회만."""
...
def insert_public_data_record(conn: sqlite3.Connection, *, source_id: str, payload: dict,
    ...
    """0937 6.2 DMS 부속서 A 2.3 수집 이력. API 스레드 소유 — 외부 HTTP 호출이
    SIAP I/O 스레드를 막으면 안 된다."""
    id_ = new_id()
    conn.execute(
...
# ── control_execution — 0937 6.5 FCS (교차 소유) ──────────────────────────
...
def insert_control_execution(conn: sqlite3.Connection, *, origin: str, install_id: str,
    ...
    """INSERT는 API 스레드가 한다 — 기록이 송신의 선행 조건이다. 미승인 규칙이면
    `trg_exec_requires_approval` 등이 `sqlite3.IntegrityError`로 막는다 — 여기서
    삼키지 않고 그대로 전파한다."""
    id_ = new_id()
    conn.execute(
...
def update_execution_result(conn: sqlite3.Connection, exec_id: str, *,
    ...
    """`SiapLink.send()`가 응답을 동기 반환하므로 API 스레드가 이 UPDATE도
    (같은 커넥션으로) 직접 수행한다."""
    conn.execute(
...
def list_device_installs_by_selector(conn: sqlite3.Connection, *, greenhouse_id: str | None = None,
    ...
    """0937 6.4-2 구역 일괄 선택 — `PATCH /device-property`. 구역의 기본 의미는
    온실 전체이며 `install_location`·`subtype`은 좁히는 선택 항목이다."""
    where: list[str] = []
    params: list = []
    joins = ""
...
def update_device_property(conn: sqlite3.Connection, install_id: str, *,
    ...
    누락 필드는 UPDATE 대상에서 빼 기존값을 보존한다(API 계약은 명시적 null을
    허용하지 않아 여기까지 오는 키는 모두 실제 값이다). 실제로 바뀐 설정 필드와
    사용자를 같은 트랜잭션의 `config_change_log`에 남긴다 — 둘 중 하나만 영속되는
    상태가 없다. 반환값은 실질 변경 여부다."""
    row = conn.execute(
...
#: 금지되는 하드코딩은 board/MCU 분기다(종류는 Subtype 레지스트리 조회로만
#: 해석한다). SQLite 테이블명은 리터럴일
...
def frame_judgement(conn: sqlite3.Connection, frame: models.FrameLog) -> str:
    """`violations`가 비어도 alert 일 수 있다(정상 NEC 알림). is_valid=0 이면 언제나
    violation, is_valid=1 이면 이 프레임을 원인으로 삼은 `alert` 행이 있는지로
    가른다(`alert.frame_id`)."""
...
#: 그대로 전파되므로, 조용히 틀린 이름을 보고하는 것보다 안전하다.
