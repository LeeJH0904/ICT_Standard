...
스레드별 연결 — "연결: 스레드별 연결. check_same_thread=False
금지"를 그대로 지킨다. 이 모듈은 커넥션을 캐싱하지 않는다 — 호출할 때마다
새 연결을 연다(짧은 트랜잭션 전제, "트랜잭션은 짧게. 프레임 1건 = 트랜잭션 1건").
"""
...
def connect(db_path: str | Path) -> sqlite3.Connection:
    ...
    아닌 새 파일)에 붙는다. PRAGMA 4종은 매 연결마다 새로 건다:
...
def init_db(db_path: str | Path, *, seed: bool = True,
    ...
    # 새 연결마다 다시 켜는 것이 이 함수의 계약이다(연결마다 켠다).
    con.execute("PRAGMA foreign_keys = ON")
