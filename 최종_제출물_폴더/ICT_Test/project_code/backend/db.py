"""
backend/db.py — DB 연결 팩토리. ★ PRAGMA는 이 파일에서만 건다.
...
스레드별 연결 — SQLite 연결은 연 스레드에서만 쓴다(`check_same_thread=False` 금지).
이 모듈은 커넥션을 캐싱하지 않고 호출할 때마다 새 연결을 연다(짧은 트랜잭션 전제 —
프레임 1건 = 트랜잭션 1건).
"""
...
def table_names(con: sqlite3.Connection) -> list[str]:
    ...
    검증기와 테스트가 공유하는 조회 — 스키마 카운트를 두 곳에서 따로 세지 않는다."""
    rows = con.execute(
...
def index_names(con: sqlite3.Connection) -> list[str]:
    """자동 생성 UNIQUE/PK 인덱스(`sqlite_autoindex_*`)는 `schema.sql`의 명시적
    `CREATE INDEX` 와 다른 근원이라 제외한다 — 명시적 인덱스만 센다."""
    rows = con.execute(
