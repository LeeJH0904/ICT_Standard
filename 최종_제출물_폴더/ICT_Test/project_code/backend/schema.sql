...
-- 1369-P1 6.1 "시간을 다루는 일관된 형식의 표기가 지정되어야 한다" — 이 구현은
--  ISO 8601 로 결정했다. 6.1은 특정 컬럼이 아니라 "시간을 다루는" 모든 표기에
--  적용되므로, 시간으로 관리하는 모든 TEXT 컬럼에 최소 GLOB 검사(연-월-일로 시작)를
--  건다. 시각·오프셋 세부 형식까지는 강제하지 않는다(과한 정규식은 그 자체가 새
--  표준 미규정 해석이 된다). SQLite는 CHECK 안에서 매크로·함수를 쓸 수 없어
--  (표현식만 허용) 컬럼마다 그대로 반복해 적는다.
...
CREATE TABLE device_info (
    ...
    -- "모델명은 장치를 고유하게 식별하는 속성으로... 모든 스마트 온실
    ...
    -- 6.2.4 "장치정보에는 장치코드, 장치이름, 장치종류, 장치모델,
    ...
    device_characteristics TEXT,                       -- 장치특성 
    CHECK (created_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*'),
    CHECK (updated_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*')
);
...
CREATE TABLE device_install_info (
    ...
    updated_at      TEXT NOT NULL,
    device_name     TEXT NOT NULL,                     -- 장치이름
    -- 6.2.5 "장치설치정보에는 장치식별자, 장치이름, 설치온실, 설치일자,
    --  설치위치 등이 포함되어야 한다" — 설치일자가 누락돼 있었다.
    --  설치온실은 이 표에 없다(관계 엔티티 device_install 이 이미 표현한다 —
    --  FK 중복을 피한다).
    -- NOT NULL만으로는 빈 문자열('')이 통과한다 — "설치일자가
    --  실제로 있다"는 6.2.5 요구를 DDL로 보장하려면 값이 있어야 한다.
    -- 빈 문자열이 아니어도 'not-a-date' 같은 임의 문자열은 막지 못한다.
    --  6.1 "시간을 다루는 일관된 형식의 표기가 지정되어야 한다" — 이
    --  구현은 ISO 8601 로 결정했다. SQLite에 정규식이 없어 GLOB으로 최소
    --  조건(YYYY-MM-DD 로 시작)만 강제한다 — 시각·오프셋 세부 형식까지
    --  DDL로 강제하지 않고, 나머지는 API 계층이 RFC 3339 전체를 검사한다.
    ...
    CHECK (installed_at <> ''),
    CHECK (installed_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*'),
    CHECK (created_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*'),
    CHECK (updated_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*')
);
...
CREATE TABLE user_info (
    ...
    CHECK (created_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*'),
    CHECK (updated_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*'),
    -- deleted_at 은 nullable(삭제되지 않은 사용자가 정상) — 있을 때만 형식을 본다.
...
CREATE TABLE device_state_data (
    ...
    CHECK (reported_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*')
);
...
CREATE TABLE config_change_log (
    ...
    CHECK (changed_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*')
);
...
CREATE TABLE public_data_source (
    ...
    license       TEXT,
    scope         TEXT,                                -- 사용 범위
    CHECK (registered_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*'),
...
CREATE TABLE public_data_record (
    ...
    CHECK (fetched_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*'),
    -- period_from/period_to 는 날짜 단독(YYYY-MM-DD)도 유효한 값이다 —
...
CREATE TABLE control_rule (
    ...
    -- 초안이 실제로 어느 경로로 만들어졌는가. origin 은 요청자의 의도이고
    ...
    target_install_id TEXT,                            -- 승인된 제어 대상 장치
    approved_at   TEXT,                                -- ★ 승인 게이트
    approved_by   TEXT,
    -- 거부도 영속 상태다. 0937 부속서 A 3.2 절차 3 "사용자는 최종 의사결정 후
    ...
    -- AI 초안 요청은 서버가 모델을 돌린 결과를 남겨야 한다.
    ...
    -- 반대 방향도 막는다. 위 CHECK 는 'AI 초안에 경로가 있는가'만 보므로
    ...
    -- 승인과 거부는 배타다. 이 CHECK 가 유일하게 그것을 막는다 -
    ...
    --  독립 반례가 없어 별도 CHECK 로 두지 않는다.
    -- created_at 은 항상 있고(NOT NULL), approved_at/rejected_at 은
...
CREATE TABLE alert (
    ...
    ack_at      TEXT,
    -- 알림을 유발한 프레임. NEC 알림(0943 8.2.1.1)을 화면에서 원본 프레임과
    ...
    -- NEC 알림은 반드시 프레임에서 유래한다(0943 8.2.1.1 NOTI_ERROR).
    ...
    CHECK (raised_at GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]*'),
...
CREATE TABLE frame_log (
    ...
    -- 이미 디코딩된 가변 요소(DEVICE_MAIN_INFO/DEVICE_PROPERTY)를
...
-- 실행 명령은 승인된 action_json 과 일치해야 한다
--  사람이 승인한 것은 '규칙의 존재'가 아니라 '그 명령'이다.
-- `<>` 는 한쪽이 NULL 이면 결과가 SQL NULL 이라 WHEN 이 참이 되지 않는다.
...
-- install_id 를 권한 불변 필드에 포함한다. 삽입 후 대상만 바꿔치기할 수 없다.
...
-- 거부 사실은 불변이다. 거부한 뒤 뒤집을 수 있으면 '사람 검토 지점'이
...
--  독립 반례가 없는 트리거를 남기면 실제로 무엇이 지탱하는지가 흐려진다.
-- reject_reason 을 감시 목록에 넣는다. 빠져 있으면 거부 사유만 사후에
