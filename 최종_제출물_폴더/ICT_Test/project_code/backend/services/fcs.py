...
담당 조항: 6.5 전부 · A.2-5·6·7 · A.3-1
진입점: execute · manual_control · query_history

순서가 중요하다 — `control_execution` 레코드를 먼저 기록하고, 성공한 경우에만
프레임을 송신한다. DB 제약(승인 게이트)이 송신의 선행 조건이 되어야 "AI 출력이
직접 구동기를 제어하지 않는다"가 코드 경로로 보장된다.

`SiapLink.send()`가 응답을 동기 반환하므로, 이 모듈(API 스레드에서 호출된다)이
`result_rsc`·`responded_at` UPDATE까지 같은 커넥션으로 직접 수행한다.
"""
...
def _send_and_finalize(conn: sqlite3.Connection, link: SiapLink, builder: FrameBuilder,
    ...
    if resp is None:
        ...
        # 0937 6.5-2 "긴급 상황시 사용자 알림"의 재전송 소진 경로. `Timeout ×
        # (Retry Count + 1)`(표 7-18) 안에 응답이 없다는 것은 구동기가 명령을
        # 받지 못했을 수도 있다는 뜻이라 사용자가 알아야 한다 — API 스레드가
        # 동기 응답 지점에서 `update_execution_result` 바로 옆에서 직접 쓴다.
