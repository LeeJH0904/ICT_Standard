...
송신 대기 및 재전송 로직을 담는다. `link.py`의 SIAP I/O 스레드가 이
...
class PendingTable:
    """응답 매칭(`Node ID` + `Message Identifier` + **`Message Type`**, 0943 7.2.2)·
    재전송(`msg_id` 유지)·타임아웃(`Timeout × (Retry Count + 1)`, 표 7-18)을
    ...
    def match(self, frame: Frame) -> bool:
        """수신 프레임을 대기 항목과 맞춘다(응답 매칭). 소비했으면
