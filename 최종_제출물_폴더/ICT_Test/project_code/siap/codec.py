...
분기 구조를 C 원본과 줄 단위로 대응시킨다 — 여기서 갈리면 C·Python 두 구현이
같은 바이트를 낸다는 상호운용성 증거가 되지 못한다.
...
51byte 슬라이딩 윈도우 대신 파이썬다운 bytearray 누적을 쓴다(C 원본과 맞추는
것은 *분기 구조*이지 임베디드 메모리 최적화가 아니다).
"""
...
SIAP_VERSION = 0x12   # v1.2, 0943 7.2.1 — firmware/core/siap_types.h SIAP_VERSION 과 동일
...
#     MSB-first, big-endian(0943 미규정 → 자체 결정).
# ═══════════════════════════════════════════════════════════════

class BitWriter:
    """bp_write 포팅. 범위 초과 시 아무것도 기록하지 않고 False 를 돌려준다
    (값이 잘려 들어가는 마스킹 래핑 금지) — bitpack.c 의 "실패 시 buf 도 *bitpos
    도 바뀌지 않는다" 계약을 그대로 지킨다."""
...
#     범위를 검사하지 않고 마스킹하면 잘못된 입력이 정상 바이트로 위장한다.
# ═══════════════════════════════════════════════════════════════
...
def decode_np(data: bytes) -> NodeProperty | None:
    """Status Reserved(0x03~0xFF, 표 7-13)면 None. C 는 이 검사를 siap_dec_feed()
    의 FIXED 상태 처리에서 하지만, Python 은 이 함수 안으로 접어 넣어 decode_frame()
    의 분기를 단순하게 유지한다."""
    r = BitReader(data)
    sw = r.read(8)
    gcg = r.read(20)
...
def decode_dmi(data: bytes) -> tuple[DeviceMainInfo | None, Violation | None]:
    """siap_decode_dmi() 포팅. Value Type(위반 6)을 Subtype(위반 7)보다 먼저 판정한다
    — 인코딩과 같은 순서."""
    r = BitReader(data)
    device_id = r.read(8)
...
def encode_dp(dp: DeviceProperty) -> tuple[bytes | None, Violation | None]:
    ...
    vt = dp.main.value_type                          # USER DEPENDENT 5필드는 main.value_type 을 따른다(표 7-15)
    try:
...
def decode_dp(data: bytes) -> tuple[DeviceProperty | None, Violation | None]:
    ...
    30byte 를 전부 읽은 뒤 판정한다(§5.6 요소는 자기완결적·고정폭)."""
...
# ═══════════════════════════════════════════════════════════════
#  3. 종류 해석 — siap_resolve_kind() 포팅(위반 조항 clause 구분 포함).
#     contracts/frame.py::resolve_kind() 는 clause 를 구분하지 않으므로 여기서
#     WIRE_CODE 를 직접 재사용해 같은 로직을 재현한다 — 같은 명세를 두 번 구현해
#     같은 결과가 나오는지가 교차 검증이다.
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
#  4. Frame 전체 인코드/디코드 — 스트리밍 디코더의 단발(비스트리밍) 판.
#     위반 판정 순서(0943 7.3.1 형식 검사 + 표 7-2/7-6/7-14):
...
def decode_frame(data: bytes, mode: Mode = "strict",
    ...
    node_known: Node ID 가 등록돼 있는지 확인하는 콜백(위반 2 판정). 코덱은 "내
    주소"를 모른다는 원칙으로 registry 를 직접 import 하지 않고, 호출자(link.py)가
    주입한다. None 이면 이 검사를 생략한다(단독 코덱 테스트·골든 벡터 재생 등)."""
...
def _decode_frame(data: bytes, mode: Mode,
    ...
    if fixed_bytes:
        ...
        if np_off is not None:
            # COMBINED_PROPERTY 3종: NODE_PROPERTY.Num. of Devices 와 Payload
            # Length 역산 N 이 다르면 거부(0943 7.3.3.4). NP 는 고정부의 마지막 8byte.
...
#     transport.py/link.py 가 바이트를 받는 대로 feed() 에 넘기면 완결된 Frame 을
#     내보낸다. 0943 이 프레임 경계 구분자·CRC 를 규정하지 않으므로(순수 바이트
#     스트림), 위반/손상 프레임 뒤에는 재동기로 다음 프레임 시작을 다시 찾는다.
# ═══════════════════════════════════════════════════════════════
...
class Decoder:
    ...
    def _resync_check(self) -> bool:
        ...
        0943 은 프레임 경계 구분자·CRC 를 규정하지 않으므로, 위반/손상 프레임 뒤에는
        바이트 스트림에서 다음 프레임 시작을 다시 찾아야 한다. 이때 후보 헤더의
        유효성을 어디까지 볼지가 관건이다.

        resolve_kind·Transmission Type·element_count 는 `decode_frame()` 자신이 검사해
        위반으로 **보고하는 바로 그 항목**이다 — 재동기 게이트에서 먼저 걸러내면 정확히
        그 위반들(예: `INVALID_FORMAT`·`INVALID_TRANSMISSION_TYPE`)이 재동기 모드 중
        "노이즈"로 오인돼 1byte 씩 삼켜지고 다시는 분류되지 않는다(연속 위반 주입에서
        두 번째부터 유실). 그래서 게이트는 Version 을 본다.

        단 Version(1byte)만으로는 오탐률이 1/256 이라, 위반 프레임이 헤더만 지우고
        남긴 잔여 payload 바이트가 우연히 `0x12`(Version)이면 게이트가 그 지점을 새
        헤더로 오인해 뒤따르는 정상 프레임을 삼킨다. 그래서 `node_known` 콜백(있으면)
        으로 **등록된 Node ID** 조건을 더한다 — 등록 노드는 보통 수십 개라 20bit Node
        ID 공간(약 100만) 대비 오탐률이 크게 낮아진다.

        **예외 — 미등록 Node ID 자체가 위반 목표인 경우**: Node ID 만 미등록이고
        나머지 구조(resolve_kind·Transmission Type·element_count)가 전부 자기충족적으로
        유효하면 후보로 인정한다. "노드를 모른다"와 "이 자리가 헤더가 아니다"는 다른
        사실이기 때문이다. `node_known=None`(단독 코덱 테스트·골든 벡터 재생)이면 Node
        ID 조건을 생략한다 — `decode_frame()` 자신도 같은 규칙을 쓴다.

        펌웨어(C)는 UART 물리 노이즈 복구가 주 목적이라 Version+resolve_kind+
        Transmission Type+element_count 4조건을 그대로 쓴다 — 표준 미규정 영역이라
        구현마다 재동기 규칙이 다를 수 있고, 정상 통신 경로에서는 두 규칙의 결과가 같다."""
        ...
        # 자기충족적으로 유효할 때만(오탐률을 4조건 수준으로 되돌림).
        if h.trans is None:
            return False
    ...
    def _try_extract(self) -> Frame | None:
        while True:
            ...
            try:
                ...
                return None
            if frame.violations:
                # 위반 프레임의 payload_len 은 신뢰할 수 없다. Version 위반(예)은
                # payload_len 자체를 전혀 검증하지 않은 채 확정되므로, 그 값으로
                # "12+payload_len" 만큼 버퍼를 지우면 이미 도착해 버퍼에 쌓여 있는
                # 다음 정상 프레임까지 통째로 삼킨다(손상된 헤더 하나가 뒤따르는
                # 정상 트래픽을 삼키는 사고). 헤더 12byte 만 지우고 재동기 모드로
                # 들어가 1byte 씩 다시 찾는다 — 느리지만 이미 버퍼에 있는 바이트를
                # 잃지 않는다.
