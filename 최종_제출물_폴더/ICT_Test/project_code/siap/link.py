...
SIAP I/O 스레드가 시리얼/소켓의 유일한 소유자다. 수신·송신·재전송을 같은 루프에서
매 회 처리한다(무한 블로킹 금지 — 그래야 무수신 중에도 API 의 송신 요청이 나간다).

`on_frame` 콜백으로 부수효과(`backend.ingest`)를 주입받는다 — `siap/` 는 `backend/`
를 import 하지 않는다는 계층 규칙을 지키면서 "I/O 스레드가 DB 반영을 직접 부른다"를
실현하는 방법이 의존성 주입이다. `run.py` 가 `link.start(..., on_frame=...)` 로 연결한다.
콜백은 DB 파일 **경로**만 받아 이 I/O 스레드 안에서 첫 호출 시점에 지연 연결한다 —
SQLite 연결은 스레드별로 열어야 하기 때문이다.

**`on_frame` 은 회신을 대신 만들지 않는다.** 회신 구성(예: REQ_SET_CONNECTION 에
어떤 NODE_PROPERTY 로 응답할지)은 표준 해석이고, 표준 해석은 프로토콜 계층에만 둔다
— `backend/` 가 그 판정을 다시 하면 두 곳에서 다르게 해석될 위험이 생긴다. 그래서
회신은 **항상** `_default_reply()` 가 만들고, `on_frame` 은 매 프레임마다 부수효과
전용으로 추가 호출될 뿐이다(반환값은 무시된다).

`on_frame` 이 없어도 내장 기본 처리기(`_default_reply`)가 항상 동작한다 —
`REQ_SET_CONNECTION` 자동 등록 + SUCCESS 회신, 그 외 노드발 Request 는 SUCCESS
회신, Notify 는 ACK. `python run.py --mode simulate` 가 `backend/` 없이도 프레임을
주고받는 이유다.

**`on_frame` 은 절대 `link.send()` 를 불러선 안 된다** — `send()` 는 이 파일의 I/O
스레드가 처리하는 큐에 넣고 기다리는데, 그 큐를 비우는 것이 지금 `on_frame` 을
호출하며 블로킹된 바로 그 스레드다(자기대기 교착).
"""
...
#: Timeout ≥ 2×wire_time, 기본 2초(N=16 501byte, 9600baud 522ms 의 2배).
#: Retry·Interval 은 표 7-18 이 값 자체를 규정하지 않아 이 프로젝트의 기본값으로
#: 둔다 — 노드별 프로파일은 RES_SET_CONNECTION 협상 결과로 갱신될 여지를 남긴다.
...
class SiapNodeLink:
    ...
    def __init__(self, gcg_id: int = 1, profile: MsgControlProfile = DEFAULT_PROFILE) -> None:
        ...
        # 실측 프레임 캡처. None 이면 비활성 — 기본 동작 불변. I/O 스레드가
    ...
    def send(self, frame: Frame, timeout: float | None = None) -> Frame | None:
        ...
        대기 상한(`Timeout × (Retry Count + 1)`)은 pending 등록 *이후* 뿐 아니라
        큐잉부터 회신까지 **호출 전체**에 걸린다 — I/O 스레드가 전송 계층 오류로
        계속 막혀 pending 등록이 미뤄져도 이 마감시각이 `out.get()` 을 무한
        블로킹에서 구한다."""
    ...
    def _write(self, data: bytes) -> bool:
        """부분 쓰기를 재시도해 전량 내보낸다(논블로킹 부분 쓰기 계약을 host 쪽에서 흡수).

        `write()` 가 0을 돌려주는 것은 "지금은 못 썼다"는 뜻이지 "더 쓸 수 없다"는
        뜻이 아니다. 유한 횟수(최대 50회, 회당 10ms)로 재시도하고, 그래도 다 못
        보내면 `tx` 를 올리지 않고 False 를 돌려준다 — 절단된 프레임을 조용히
        성공으로 보고하지 않는다."""
    ...
    def _io_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            # 전송 계층 예외(상대가 연결을 끊는 등)가 나도 이 회차의 ②·③(송신 큐
            # 처리·pending 만료)은 반드시 계속 돈다 — 그래야 `send()` 가 등록·상한
            # 계산에 도달해 무한 대기하지 않는다.
            try:
            ...
            if chunk:
                for frame in self._decoder.feed(chunk):
                    # decode_frame() 은 바이트를 해석할 뿐 수신 시각을 모른다
                    # (자기 완결적 순수 함수 — 골든 벡터 재생·단독 코덱 테스트의
                    # 결정론을 위해 일부러 그렇다). 실제 수신 시각은 디코더가
                    # 완결된 프레임을 내보낸 "지금"만이 안다 — 여기서 한 번만
                    # 채운다. Frame 은 frozen 이라 replace() 로 새 값을 만든다.
                    frame = dataclasses.replace(frame, t=time.time())
                    if self._capture is not None:
                        self._capture("rx", frame.raw)   # 위반 프레임의 원시 바이트도 그대로
                    self._stats["rx"] += 1
                    if frame.violations:
    ...
    def _dispatch(self, frame: Frame) -> Frame | None:
        """회신은 항상 `_default_reply()` 가 만든다(표준 해석은 프로토콜 계층에만
        둔다). `on_frame` 은 설정돼 있으면 부수효과(예: `backend.ingest.handle` 의
        DB 반영)로 추가 호출될 뿐이고, 반환값은 쓰지 않는다."""
    ...
    def _apply_registry_effects(self, frame: Frame, reply: Frame | None) -> None:
        ...
        무관하게 이 한 곳에서만 일어난다 — `on_frame` 이 `backend.ingest.handle`
        로 교체돼도 `registry()`/`devices()` 가 실제 연결 상태를 반영하도록
        보장하고, 갱신 지점을 하나로 좁힌다.

        회신 Frame 자체에서 등록 내용을 읽는다 — `build.res_set_connection()` 은
        항상 유효한 `node_property` 를 채우므로, 어느 경로가 회신을 만들었든 이
        판정은 동일하게 성립한다."""
        ...
        if (frame.kind is MsgKind.REQ_SET_CONNECTION and reply is not None
            ...
            # REQ_SET_CONNECTION 은 페이로드가 없어(LAYOUT (0,0)) 디바이스 구성을
            # 실을 수 없다. 노드가 연결 성공 뒤 이 메시지로 전체 구성을 선언하면
            # 그때 registry 를 갱신한다(0943 8.1.3.3, "ALL"이므로 전체 교체).
    ...
    def _replace_profile(self, profile: MsgControlProfile) -> None:
        """링크 대기 상한과 PendingTable 재전송 정책을 한 갱신점에서 교체한다.
        send()의 프로파일 읽기도 같은 잠금으로 직렬화한다."""
        with self._profile_lock:
            self._pending.update_profile(profile)
            self._profile = profile

    def _default_reply(self, frame: Frame) -> Frame | None:
        """`backend/` 없이도 동작하는 최소 기본 처리기 — `REQ_SET_CONNECTION` 은
        SUCCESS 회신(등록은 `_apply_registry_effects()` 가 한다), 그 외 노드발
        Request 는 SUCCESS 회신, Notify 는 ACK."""
        if frame.header is None:
            return None
        if frame.violations:
