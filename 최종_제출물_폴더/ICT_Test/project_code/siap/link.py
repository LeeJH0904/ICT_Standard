...
SIAP I/O 스레드가 시리얼/소켓의 유일한
소유자다. 수신·송신·재전송을 같은 루프에서 매 회 처리한다(무한 블로킹 금지 —
그래야 무수신 중에도 API 의 송신 요청이 나간다).

`on_frame` 콜백으로 부수효과(`backend.ingest`)를 주입받는다 — `siap/`
은 `backend/` 를 import 하지 않는다는 계층 규칙을 지키면서도,
I/O 흐름이 그리는 "I/O 스레드가 `ingest.handle()`을
직접 부른다" 동작을 실현하는 유일한 방법이 의존성 주입이다 — `run.py`가
`link.start(..., on_frame=_make_on_frame(db_path))`로 연결한다.
...
ProgrammingError` 로 죽는다("스레드별 연결",
...
표준 해석은 프로토콜 계층에만 있다 — `backend/` 가 그 판정을
...
SUCCESS 회신, Notify는 ACK. `python run.py --mode simulate`가
...
#: Timeout ≥ 2×wire_time, 기본 2초(N=16 501byte, 9600baud
...
class SiapNodeLink:
    ...
    def __init__(self, gcg_id: int = 1, profile: MsgControlProfile = DEFAULT_PROFILE) -> None:
        ...
        # 실측 캡처. None이면 비활성 — 기본 동작 불변. I/O 스레드가
