...
replay·simulate 모두 TCP 소켓만 쓴다 — 외부 도구 의존 없이 심사자 OS 와 무관하게
동작하기 위해서다. 이 파일은 **클라이언트 쪽 배관만** 담당한다 — 반대편(로그 재생
서버·가상 노드 서버)은 `sim/` 의 몫이며, `SocketTransport` 자체는 임의의 TCP
서버에 접속해 동작한다.
"""
...
#: hardware 는 장치 경로, replay/simulate 는 (host, port).
...
class Transport(Protocol):
    ...
    def read(self, max_bytes: int, timeout: float) -> bytes:
        """`timeout` 초 안에 1byte 도 못 읽으면 `b""` 를 돌려준다 — 무수신에도
        I/O 루프가 계속 돈다."""
        ...

    def write(self, data: bytes) -> int:
        """실제로 쓴 바이트 수. 부분 쓰기를 허용한다(`siap_io_write_fn` 과 동일
        계약) — 호출자가 남은 바이트를 재시도한다."""
        ...


class SerialTransport:
    """hardware 모드. `pyserial` 은 이 프로젝트의 직접 의존성 3종 중 하나다.
    import 는 지연시킨다 — replay/simulate 만 쓰는 심사 환경에서 `pyserial` 이
    없어도 이 파일 자체(및 이 클래스 정의)는 로드돼야 한다."""
...
class SocketTransport:
    ...
    `recv()` 가 돌려주는 `b""` 는 "타임아웃(아직 안 왔다)"과 "상대가 연결을 정상
    종료했다(FIN)"를 구분하지 않는다 — link.py 의 폴링 루프가 어느 쪽이든 다음
    회차에 다시 시도한다."""
...
def open_transport(run_mode: str, **opts) -> Transport:
    ...
    opts:
      ...
      replay/simulate — `host`(기본 127.0.0.1) · `socket_port`(기본 포트는 위 표).
