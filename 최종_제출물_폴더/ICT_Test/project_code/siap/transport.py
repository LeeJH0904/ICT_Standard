"""
siap/transport.py — 전송 계층 3모드 (아키텍처 설계서 §5, CLAUDE.md §7).

심사자에게는 라즈베리파이도 MCU 도 없다 — 교체 대상은 전송 계층 한 곳뿐이다.

  hardware : 실제 시리얼 포트 (pyserial)
  replay   : TCP 소켓 클라이언트 — 실측 로그 재생 서버(`sim/replayer.py`, 단계 4)에 접속
  simulate : TCP 소켓 클라이언트 — 가상 노드 서버(`sim/virtual_node.py`, 단계 4)에 접속

replay·simulate 모두 TCP 소켓만 쓴다 — `socat` 등 외부 도구 의존 금지
(CLAUDE.md §1-8), 심사자 OS 무관성이 이유다. 이 파일은 **클라이언트 쪽
배관만** 담당한다 — 반대편(로그 재생 서버·가상 노드 서버)은 `sim/`(단계 4)의
몫이며, 이 단계에서는 그 서버가 없어도 `SocketTransport` 자체는 임의의
...
#: CLAUDE.md §7 URLS 표 그대로. hardware 는 장치 경로, replay/simulate 는 (host, port).
...
class Transport(Protocol):
    ...
    def read(self, max_bytes: int, timeout: float) -> bytes:
        """`timeout` 초 안에 1byte 도 못 읽으면 `b""` 를 돌려준다(아키텍처
        설계서 §3.1-b "read timeout 이 지나면 b"" 를 돌려주고 넘어간다" —
        무수신에도 I/O 루프가 돈다)."""
        ...

    def write(self, data: bytes) -> int:
        """실제로 쓴 바이트 수. 부분 쓰기를 허용한다(`siap_io_write_fn` 과
        동일 계약, 펌웨어 설계서 §5.8) — 호출자가 남은 바이트를 재시도한다."""
        ...


class SerialTransport:
    """hardware 모드. `pyserial` 은 CLAUDE.md §4.3 이 허용한 의존성 3종 중
...
def open_transport(run_mode: str, **opts) -> Transport:
    ...
    opts:
      ...
      replay/simulate — `host`(기본 127.0.0.1) · `socket_port`(기본은 CLAUDE.md §7 표.
