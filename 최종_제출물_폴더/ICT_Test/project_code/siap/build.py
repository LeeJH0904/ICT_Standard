...
두 묶음:
  (1) 게이트웨이발 Request 5종 — `link.send()` 경유. msg_id 는 이 파일이
      발번한다(0943 7.2.2 "0~65535 순환, 송신마다 +1, 65535 다음 0". 초기값 0 —
      0도 유효한 순번이다. 노드측 규칙과 대칭이 되도록 게이트웨이도 같은 규약).
  (2) 노드발 메시지에 대한 즉시 회신 7종. msg_id·GCG ID·Node ID 를 원본 Frame
      에서 그대로 복사한다(7.2.2) — 새로 발번하지 않는다.
...
class MsgIdAllocator:
    """0943 7.2.2 원문 — "Message Identifier는 … '0'에서 '65535'까지 사용할 수
    있다. 일련번호는 데이터 전송 시마다 +1을 하며 만료되면 0부터 다시 시작한다."
    0을 건너뛰지 않는다 — uint16 덧셈이 0xFFFF 다음 0 으로 감아 표준 문구와 그대로
    맞고, node_state.c 의 노드측 규칙과도 대칭이다."""
...
class FrameBuilderImpl:
    ...
    def _reply_header(self, req: Frame, kind: MsgKind, payload_len: int) -> Header:
        """msg_id·GCG ID·Node ID 를 원본에서 복사한다(0943 7.2.2). 새로 발번하면
        노드가 중복 요청으로 처리한다."""
    ...
    def _lookup_device_kind(self, node_id: int, device_id: int) -> tuple[DevType, int]:
        ...
        도 요구하는데 그 둘을 전달할 통로가 Protocol 시그니처에 없다. 그래서
        `registry` 에서 그 노드의 실제 DEVICE_MAIN_INFO 를 device_id 로 찾아 채운다.

        registry 가 없거나 그 노드에 `device_id` 가 없으면 조용히 넘어가지 않고
        실패시킨다 — 확인 못 한 디바이스 종류를 지어내 제어 프레임에 실으면,
        엉뚱한 종류로 해석하는 노드에 잘못된 제어값이 전달될 수 있기 때문이다."""
    ...
    def res_set_connection(self, req: Frame, rsc: RSC,
        ...
        if rsc == RSC.SUCCESS:
            ...
            # 0943 은 실패 시 페이로드 형태를 규정하지 않는다. 이 구현은 LAYOUT
            # (고정부=RSC+NODE_PROPERTY, N=0 허용)을 그대로 따르고 자리표시
            # NodeProperty(N=0)를 채운다 — "RSC 만 싣는다"는 "실제 디바이스
            # 데이터를 담지 않는다"는 뜻으로 해석했다. LAYOUT 을 바꾸지 않는 한
            # RES_SET_CONNECTION 은 항상 RSC+NODE_PROPERTY(9byte) 고정부를 갖는다.
