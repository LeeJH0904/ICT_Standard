#!/usr/bin/env python3
"""F-113 — SIAP_WUR / -Werror=unused-result 회귀 검사.
...
(F-113 처리 중 발견) — mingw32-make 는 POSIX sh 를 못 찾으면 cmd.exe 로
...
F-114 — "컴파일이 실패했다"만으로는 부족하다. 스니펫 파일이 삭제되거나
...
# F-045 원칙과 같은 2중 방어 — 이 스크립트 자신의 print() 는 ASCII 뿐이라
...
#
# F-114 재발(단계 2b) — siap_frame.h 로 SIAP_WUR 가 새로 9개 옮겨갔는데
...
# 않았다 — bitpack.c/.h 에서 F-113/F-114 가 고쳤던 것과 같은 종류의 구멍이
# 새 헤더에서 그대로 재발한 것이다.
SNIPPETS = (
...
def _run(cmd: list[str]) -> tuple[int, str]:
    """F-045/F-096/F-102 와 같은 부류 — text=True 는 로케일 기본 인코딩
    (한국어 Windows에서는 cp949)으로 디코딩한다. gcc 진단이 소스 주석의
    UTF-8 문자(em dash 등)를 그대로 인용하면 cp949로 못 읽어
...
def main(argv: list[str]) -> int:
    ...
    # F-114 선검사 ① — 컴파일러 자체가 실행 가능한가. 이게 안 되면 아래
    # 모든 판정이 무의미하다(모든 컴파일이 "실패"로 보여 거짓 OK 를 낼 수 있다).
    cc_rc, cc_out = _run([cc, "--version"])
    if cc_rc != 0:
        print(f"FAIL(F-114): compiler '{cc}' is not runnable (rc={cc_rc}): {cc_out[-300:]}")
        return 1
    ...
    for label, src_name in SNIPPETS:
        src = HERE / src_name
        # F-114 선검사 ② — 스니펫 자체가 없으면 gcc 는 'No such file' 로
        ...
        # F-114 핵심 — 컴파일이 실패했다는 사실만으로 "정상"이라 판정하지
    ...
    if regressed or unusable:
        for label in regressed:
            print(f"FAIL(F-113): {label}() return-value discard did not fail to "
                  f"compile - SIAP_WUR / -Werror=unused-result regression")
        for msg in unusable:
            print(f"FAIL(F-114): {msg} - check_wur cannot confirm the "
                  f"unused-result contract is enforced")
        return 1

    print("OK(F-113/F-114): bp_write()/bp_write_f32() return-value discard "
