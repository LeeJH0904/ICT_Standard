 ...
 * IEEE-754 비트 인코딩만 담당한다(펌웨어 설계서 §7.2).
 *
 ...
 *
 * 합성 데이터 금지(CLAUDE.md §1-1)는 "값을 지어내는 것"을 금지한다 — 실측 ADC
 ...
 * (CLAUDE.md §3.5 FLOAT 결정). 캐스팅/마스킹이 아니라 비트 복사여야 한다. */
 ...
 * 보드 계층의 자유이며 core/ 는 바뀌지 않는다(§1-5). */
 ...
 * 두고 NOTI_ERROR(ERROR_DEVICE_INTERFACE)를 보낸다(§7.2). 임계는 배선 의존이며
