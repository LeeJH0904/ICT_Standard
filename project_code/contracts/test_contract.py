...
# 11. F-014 회귀 — Header가 미정의 Transmission Type을 보존하는가
h3 = Header(0x12, 0x0800, 3, 1, 14, 1, 3)          # 표 7-6 미정의값 0x03
t("F-014 trans_type=0x03 저장 가능", h3.trans_type == 3)
t("F-014 미정의값 해석 → None", h3.trans is None)
t("F-014 정상값 해석", Header(0x12,0,0,1,0,1,3).trans is TransType.UNICAST)
t("F-014 resolve_trans_type 0/1/2 정상",
  [resolve_trans_type(i) for i in (0,1,2)] == [TransType.UNICAST, TransType.MULTICAST, TransType.BROADCAST])
t("F-014 msg_type도 raw int 유지", isinstance(h3.msg_type, int) and h3.msg_type == 0x0800)

# 12. F-022 회귀 — USER DEPENDENT 5필드가 int도 허용하는가
...
t("F-022 INT 디바이스의 경계값이 int로 유지",
...
t("F-022 FLOAT 디바이스의 경계값이 float로 유지", isinstance(dp_f.precision, float))

# 13. F-025 회귀 — siap_iface 가 패키지·스크립트 양쪽에서 import 되는가
...
try:
    ...
    _ok = False
t("F-025 siap_iface import + Protocol 노출", _ok)

# 14. 명세서 예시 프레임과 대조
#     F-025/이관 — spec_examples.json은 project_docs/siap/ 의 설계 산출물이며
#     이관 대상이 아니다(개발_착수_지시서 §3.1 범위). 이 파일이
#     project_docs/contracts/ 든 project_code/contracts/ 든 같은 파일을 찾도록
#     고정 상대경로 대신 저장소 루트까지 위로 올라가며 찾는다
#     (golden_verify.py find() 와 동일한 패턴).
import json
...
t("F-040 해석 불가(None)에는 회신하지 않는다", reply_kind(None) is None)
...
t("F-086 두지 않은 게이트웨이발 빌더 8종의 사유가 계약에 적혀 있다",
...
#     교체해도 여기서 FAIL 이 나야 한다 — 그게 F-106 의 반례였다.
...
try:
    ...
    _FakeLink = None
    _fl_load_ok = False
t("F-106 fake_link.py 로드 + FakeSiapLink 클래스 노출", _fl_load_ok)
...
if _fl_load_ok:
    try:
        ...
        _link = None
t("F-106 FakeSiapLink() 인스턴스화 가능", _link is not None)

_missing = [m for m in _SIAP_LINK_METHODS if not (_link is not None and callable(getattr(_link, m, None)))]
t("F-106 SiapLink 메서드 7종이 실제로 존재하고 callable", not _missing, str(_missing))
...
t("F-106 SiapLink 메서드 7종을 최소 정상 입력으로 호출 (반환형 확인)",
  not _call_bad, "; ".join(_call_bad))

# 18. F-110 회귀 — F-106 은 "메서드가 존재하고 최소 입력으로 호출된다"까지만
#     봤다. `start` 에서 `**opts` 를, `send` 에서 `timeout` 을 빼거나 `recv()`
#     가 list 를 그대로 반환해도 F-106 은 그대로 60/60 이었다 — 이름·개수가
...
t("F-110 SiapLink 메서드 7종의 파라미터 이름·kind·기본값이 Protocol 과 호환",
...
if _link is not None and not _missing:
    ...
    # F-110 의 핵심 — list(...)로 감싸면 list 도 통과한다. Iterator 계약은
...
t("F-110 start(**opts)·send(timeout=) 실호출 및 recv() 의 Iterator 계약",
