...
(CLAUDE.md §4.4 "C 를 먼저 쓰고 Python 으로 옮긴다" — 반대 방향은 AVR 제약을
놓친다). 분기 구조를 C 원본과 줄 단위로 대응시킨다(펌웨어 설계서 §5.3) — 여기서
...
51byte 슬라이딩 윈도우 대신 파이썬다운 bytearray 누적을 쓴다(펌웨어 설계서
§5.3 "C 를 파이썬답게 다시 쓰지 않는다"는 *분기 구조*를 말하는 것이지, 임베디드
...
#     SIAP 메시지 명세서 §10.4 F-044·F-047·F-055·F-058 — 범위를 검사하지 않고
...
def pack_float(value) -> int:
    """FLOAT → IEEE-754 single 32bit raw. inf/nan/표현 불가 값을 거부한다(F-055)."""
    try:
        v = float(value)
    except (TypeError, ValueError, OverflowError) as e:      # F-058 — 10**400 등
...
def encode_np(np: NodeProperty) -> bytes:
    if int(np.status) > int(Status.UNKNOWN):        # F-127 — 표 7-13 Status Reserved
...
def decode_np(data: bytes) -> NodeProperty | None:
    """F-127 — Status Reserved(0x03~0xFF)면 None. C 의 siap_decode_np() 자체는
...
def encode_dp(dp: DeviceProperty) -> tuple[bytes | None, Violation | None]:
    ...
    vt = dp.main.value_type                          # F-022 — USER DEPENDENT 5필드는 main.value_type 을 따른다
    try:
...
def _resolve_kind_with_clause(msg_type: int, payload_len: int, mode: Mode):
    ...
    if len(cands) == 1:                                # 단일 후보 — element_count 무관하게 확정 (F-119)
        return cands[0], None
...
def decode_frame(data: bytes, mode: Mode = "strict",
    ...
    if n is None:                                                           # 위반 3(단일 후보, F-119/B02 류)
    ...
    if fixed_bytes:
        if kind in _KIND_HAS_LEADING_RSC:                                   # F-127 — 표 7-10 RSC Reserved
            rsc_raw = fixed[0]
            if rsc_raw > int(RSC.INVALID_FORMAT):
                return _violation(RSC.INVALID_FORMAT, "INVALID_FORMAT", "7.3.1",
                                   f"RSC=0x{rsc_raw:02X} 는 RESERVED")
            rsc = RSC(rsc_raw)
        if kind is MsgKind.NOTI_ERROR:                                      # F-127 — 표 7-12 NEC Reserved
            nec_raw = fixed[0]
        ...
        if np_off is not None:
            # F-126 — COMBINED_PROPERTY 3종: NODE_PROPERTY.Num. of Devices 와
            ...
            if node_property is None:                                       # F-127 — 표 7-13 Status Reserved
                return _violation(RSC.INVALID_FORMAT, "INVALID_FORMAT", "7.3.1",
    ...
    if elem_bytes and n:
        if elem_bytes == DID_BYTES:
            ...
            for i in range(n):
                ...
                if dmi is None:                                             # 위반 6·7 — 첫 위반에서 중단(§5.6)
...
# ═══════════════════════════════════════════════════════════════
#  5. 스트리밍 디코더 — siap_dec_t 포팅(펌웨어 설계서 §5.1/§5.7).
#     transport.py/link.py 가 바이트를 받는 대로 feed() 에 넘기면 완결된
#     Frame 을 내보낸다. 프레임 경계 구분자가 없는 표준의 결함(SIAP 메시지
#     명세서 §6 #10)에 대한 자체 대응이 재동기 4조건이다.
#
#     알려진 범위 — 이번 단계는 "정상적으로 이어지는 바이트열에서 프레임을
#     추출"과 "위반 프레임 뒤 재동기 진입"까지를 구현한다. 실제 바이트 유실
#     주입 시나리오(T_gap 경계값 등)의 정밀 검증은 sim/inject.py·
#     tools/mode_verify.py(단계 4)의 몫이다 — 여기서는 그 훅(on_gap)만 제공한다.
# ═══════════════════════════════════════════════════════════════
...
class Decoder:
    ...
    def on_gap(self) -> None:
        """T_gap(20ms, §5.7) 이상 무입력을 관측했을 때 호출한다. 헤더 대기
    ...
    def _resync_check(self) -> bool:
        ...
        둘을 본다(F-146 최초 완화, F-151 로 조건 하나 보강).

        펌웨어(C, `siap_frame.c`)와 §5.7 문서의 원안은 Version + resolve_kind +
        ...
        **보고하는 바로 그 항목**이다 — 재동기 게이트에서 먼저 걸러내면
        ...
        직후에는 (F-140 에 따라) 항상 재동기 모드로 들어가므로, 시연
        시나리오 §3.1 의 연속 주입(X01→X03→X05→X06→X07)에서 두 번째부터는
        전부 유실됐다(실측: msg_id [50,55,56]만 도달, [52,54] 소실).

        **F-146 직후 Version 단독 조건의 결함(F-151)**: 위반 프레임을 헤더
        12byte 만 지우고(F-141) 재동기에 들어가면, 그 위반 프레임이 선언한
        ...
        프레임 하나가 사라진다(F-140 이 막으려던 것과 같은 종류의 사고가
        ...
        오탐률이 극적으로 낮아진다. 시연 시나리오 §3.1 의 X01·X03·X05·X06·
        X07 은 전부 `Node ID=3`(등록된 노드)을 선언하므로 이 조건을 항상
        통과한다 — F-146 이 고친 연속 주입 판정은 그대로 유지된다.
        ...
        **단, `node_known` 실패를 무조건 거부로 쓰면 X02(미등록 Node ID
        ...
        F-146 이전 4조건 오탐률(약 2⁻²²)과 동일한 수준으로 낮다.
        ...
        유지한다(§5.7 마지막 문단이 이미 "상대 구현이 다른 규칙을 쓰면
        재동기 시점이 달라진다"고 명시) — CLAUDE.md §3.5 결정 표 참조."""
        ...
        # 자기충족적으로 유효할 때만(F-151 재발 방지, 오탐률을 4조건
        # 수준으로 되돌림).
        if h.trans is None:
            return False
    ...
    def _try_extract(self) -> Frame | None:
        while True:
            ...
            try:
                ...
                return None
            if frame.violations:
                # F-140 — 위반 프레임의 payload_len 은 신뢰할 수 없다. Version
