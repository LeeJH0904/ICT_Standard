# DB 스키마 설계서 — 스마트 온실 데이터

> **근거 표준**: TTAK.KO-10.1369-Part1 7.2 논리적 모델(주) / TTAK.KO-10.0937 6장·부속서 A / TTAK.KO-10.0943 7장 연동
> **DBMS**: SQLite 3 (파일 단일, 오프라인 재현 최적)
> **담당**: B (SW) / **일정**: D2
> **검증**: DDL 실행 성공 + 표준 유래 제약 **109종 동작 테스트 109/109 통과** (2026-08-11 실측, 당시 98종 · 2026-08-07, 101종 · 2026-08-10 오전, 103종 · 2026-08-10, 108종 · 2026-08-11 — 단계 5에서 F-158·F-159·F-162로 설치일자 NOT NULL·빈 문자열 차단·model_name UNIQUE 검사 3건 추가 후, F-166으로 설치일자 형식(ISO 8601) 검사 2건 추가, F-184로 시간 형식(ISO 8601) 검사를 installed_at 하나에서 created_at·updated_at·issued_at 등 나머지 시간 컬럼으로 넓히며 5건 추가, F-185로 장치정보 장치특성 컬럼 존재 검사 1건 추가)
...
| # | 원칙 | 근거 |
|---|---|---|
| 1 | **1369-Part1 7.2 ER 다이어그램을 직역한다.** 엔티티·속성·관계를 임의로 병합하거나 정규화하지 않는다 | 심사자가 그림 7-2·7-3·7-4와 스키마를 1:1 대조할 수 있어야 한다 |
| 2 | 표준이 규정한 **무결성 제약을 DDL로 강제**한다. 애플리케이션 레벨 검증에 맡기지 않는다 | 1369-Part1에는 예시 데이터가 없다. 제약 테스트가 서버측 표준 준수의 유일한 증거다 |
| 3 | 식별자는 **UUID(TEXT)**, 시간은 **ISO 8601(TEXT)** | 6.1 — *"식별자의 표기는 ITU-T X.667 권고안을 준용"*, *"시간을 다루는 일관된 형식의 표기가 지정되어야 한다"* |
| 4 | 값·단위·오차범위·유효범위를 **함께** 저장한다 | 6.3.2 — *"측정형 데이터는 데이터의 단위가 관리되어야 한다 / 유효범위가 관리되어야 한다 / 오차범위가 관리되어야 한다"* |
| 5 | 0943 연동 컬럼은 **확장임을 명시**하고 접두사 `siap_`로 격리한다 | 1369-Part1 미규정 사항. 두 표준을 잇는 지점이므로 문서화 가치가 크다 |
...
| 구분 | 개수 | 근거 |
|---|---|---|
| A. 설정형 데이터 | 5 | 1369-P1 6.2 / 7.2.2.2~7.2.2.6 |
| B. 설정형 데이터 간 관계 | 4 | 1369-P1 7.2.2.7~7.2.2.10 |
| C. 측정형 데이터 | 10 | 1369-P1 6.3 / 7.2.3 |
| D. 설정형 ↔ 측정형 관계 | 3 | 1369-P1 7.2.4.2~7.2.4.4 |
| E. 변경 이력 | 1 | 1369-P1 6.2.1 |
| F. 서비스 계층 | 6 | 0937 6.2·6.3·6.4·6.5 / 부속서 A |
| G. 프레임 로그·검증 | 2 | 0943 7장 / 기능 2 |

**1369-Part1 유래 23개, 0937 유래 6개, 0943·기능2 유래 2개.**

### 2.1 전체 목록

| # | 테이블 | 한글명 | 표준 조항 |
|---|---|---|---|
| A-1 | `farm_info` | 농장 정보 | 1369-P1 6.2.2 / 7.2.2.2 |
| A-2 | `greenhouse_info` | 온실 정보 | 1369-P1 6.2.3 / 7.2.2.3 |
| A-3 | `device_info` | 장치 정보 | 1369-P1 6.2.4 / 7.2.2.4 |
| A-4 | `device_install_info` | 장치설치 정보 | 1369-P1 6.2.5 / 7.2.2.5 |
| A-5 | `user_info` | 사용자 정보 | 1369-P1 6.2.6 / 7.2.2.6 |
| B-1 | `greenhouse_own` | 온실 소유 | 1369-P1 7.2.2.7 |
| B-2 | `greenhouse_manage` | 온실 관리 | 1369-P1 7.2.2.8 |
| B-3 | `device_install` | 장치 설치 | 1369-P1 7.2.2.9 |
| B-4 | `device_manage` | 장치 관리 | 1369-P1 7.2.2.10 |
| C-1 | `device_state_data` | 장치상태 데이터 | 1369-P1 7.2.3.2 |
| C-1-a | `dsd_window_opener` | └ 창 개폐기 | 1369-P1 6.3.4.2 / 그림 7-3 |
| C-1-b | `dsd_insulation_cover` | └ 보온 덮개 | 1369-P1 6.3.4.3 / 그림 7-3 |
| C-1-c | `dsd_irrigation_pump` | └ 관수펌프 | 1369-P1 6.3.4.5 / 그림 7-3 |
| C-1-d | `dsd_irrigation_valve` | └ 관수밸브 | 1369-P1 6.3.4.6 / 그림 7-3 |
| C-1-e | `dsd_fan` | └ 송풍기 | 1369-P1 6.3.4.4 / 그림 7-3 |
| C-1-f | `dsd_cooling_heater` | └ 냉난방기 | 1369-P1 6.3.4.7 / 그림 7-3 |
| C-2 | `env_state_data` | 환경상태 데이터 | 1369-P1 7.2.3.3 |
| C-2-a | `env_measurement` | └ 환경 측정치 | 1369-P1 6.3.3 / 그림 7-3 |
| C-3 | `operating_env` | 작동 환경 | 1369-P1 7.2.3.4 |
| D-1 | `device_state` | 장치상태 관계 | 1369-P1 7.2.4.2 |
| D-2 | `env_measure` | 환경측정 관계 | 1369-P1 7.2.4.3 |
| D-3 | `greenhouse_env` | 온실환경 관계 | 1369-P1 7.2.4.4 |
| E-1 | `config_change_log` | 설정형 변경 이력 | 1369-P1 6.2.1 |
| F-1 | `public_data_source` | 공공데이터 메타정보 | 0937 6.2 / 부속서 A 2.3 |
| F-2 | `public_data_record` | 공공데이터 수집 이력 | 0937 부속서 A 2.3 |
| F-3 | `control_model` | 제어 모델 메타정보 | 0937 6.3 |
| F-4 | `control_rule` | 제어 규칙 + 승인 게이트 | 0937 6.3 / 부속서 A 3.3 |
| F-5 | `control_execution` | 제어 실행 이력 | 0937 6.5 / 부속서 A 3.3 |
| F-6 | `alert` | 알림 | 0937 6.4 / 6.5 |
| G-1 | `frame_log` | 프레임 로그 | 0943 7장 / 기능 2 |
| G-2 | `frame_violation` | 표준 위반 내역 | 0943 7.3.1·7.3.2 / 기능 2 |
...
```
  [농장 정보]────< 온실 소유 >────[온실 정보]────< 장치 설치 >────[장치설치 정보]
       │                              │                                │
       │ 소유자(FK)                   │                                │ 장치정보(FK)
       │                              │                                │
  [사용자 정보]───< 온실 관리 >───────┘                          [장치 정보]
...
```
  창개폐기 / 보온덮개 / 관수펌프 / 관수밸브 / 송풍기 / 냉난방기
        └──────────────┬──────────────┘  (일반화)
                       ▼
              [장치상태 데이터]───< 작동 환경 >───[환경상태 데이터]
              식별자(PK)                          식별자(PK)
              상태보고시간                         측정시간 / 측정위치
                                                        ▲  (일반화)
                          ┌─────────────────────────────┘
        온도 / 습도 / 일사 / CO2 / 풍향 / 토양수분장력 / EC / pH  ← 측정값·단위·오차범위·유효범위
        감우                                                    ← 측정값만
```

### 3.3 설정형 ↔ 측정형 (그림 7-4 직역)

```
  [장치설치 정보]───< 장치상태 >───[장치상태 데이터]
        │
        └──────────< 환경측정 >───[환경상태 데이터]
                                        ▲
  [온실 정보]──────< 온실환경 >──────────┘
...
그림 7-3은 장치상태·환경상태 데이터를 서브타입을 갖는 **일반화 구조**로 정의한다. 구현 방식을 다음과 같이 나눴다.

| 대상 | 방식 | 근거 |
|---|---|---|
| **장치상태 데이터 6종** | 공통 테이블 + **서브타입 테이블 6개** (Class Table Inheritance) | 그림 7-3에서 6종의 속성이 실제로 다르다. 창개폐기(개폐정도)·보온덮개(각도)·관수펌프(압력+분사도)·송풍기(전원+바람세기)·냉난방기(전원+온도+바람세기). 병합하면 표준 대조가 불가능해진다 |
| **환경상태 데이터 9종** | 공통 테이블 + **통합 서브타입 테이블 1개** (`subtype` 판별자) | 그림 7-3에서 8종이 `측정값·단위·오차범위·유효범위`로 **구조가 완전히 동일**하고 감우만 `측정값` 단독이다. 9개 테이블로 쪼개면 동일 DDL이 8번 반복된다. 감우의 차이는 CHECK 제약으로 강제했다 |

> 환경상태를 통합한 판단은 `docs/standard-mapping.md`에 근거와 함께 명시한다. 표준을 임의로 축약한 것이 아니라 **구조 동일성이 확인된 8종을 판별자 컬럼으로 대체**했음을 밝힌다.

### 4.1-a 카디널리티는 7.1 개념적 모델에서 온다

7.2의 관계 서술은 *"구성 식별자가 모두 일치하는 2개 이상의 관계 데이터가 존재할 수 없다"* 는 **쌍 유일성**만 규정한다. 이것만 구현하면 온실 1개가 농장 2곳에 동시에 속할 수 있다.

**실제 카디널리티는 7.1 개념적 모델에 명시되어 있다.**

| 조항 | 원문 | 제약 |
|---|---|---|
| 7.1(1) | *"특정한 1개의 온실은 1개의 농장에만 포함된다"* | `greenhouse_own` UNIQUE(greenhouse_id) |
| 7.1(3) | 온실 N:1 사용자 | `greenhouse_manage` UNIQUE(greenhouse_id) |
| 7.1(4) | *"1개의 장치는 1개의 온실에 설치될 수 있다"* | `device_install` UNIQUE(install_id) |
| 7.1(5) | 온실 1:N 환경상태 | `greenhouse_env` UNIQUE(env_state_id) |
| 7.1(7) | *"설치된 장치들은 1명의 사용자에 의해 관리된다"* | `device_manage` UNIQUE(install_id) |
| 7.1(8) | *"1개의 설치된 장치는 N개의 장치상태를 가진다"* | `device_state` UNIQUE(device_state_id) |
| 7.1(9) | 장치 1:N 환경상태 | `env_measure` UNIQUE(env_state_id) |
| 7.1(10) | 장치상태 1:N 환경상태 | `operating_env` UNIQUE(env_state_id) |

7.2만 읽고 구현하면 8건 전부가 미강제 상태로 남는다. **개념적 모델과 논리적 모델을 함께 읽어야 하는 이유다.**

> **F-156·F-164 — `operating_env` 결속 시점의 표준 미규정 사항**: 7.1(10)이 요구하는
> `UNIQUE(env_state_id)`는 DDL로 강제되지만, **어느 프레임에서 어느 장치상태에
> 결속할지는 0943·1369-P1 어느 쪽도 규정하지 않는다.** `NOTI_DEVICE_VALUE` 프레임
> 하나에 장치상태가 2건 이상 함께 오면(예: 관수펌프 압력+분사도처럼 물리량이 여럿인
> 서브타입이 둘 이상) 프레임 구조만으로는 어느 것과 짝지어야 하는지 정할 수 없다.
> 구현은 **장치상태가 정확히 1건일 때만 결속하고, 2건 이상이면 그 프레임의
> `operating_env`는 전부 건너뛴다** — 모호한 짝을 추정하지 않는다(정본은 CLAUDE.md
> §3.5, 구현은 `backend/ingest.py::_handle_device_value`).

### 4.2 0943 연동 지점

1369-Part1에는 노드/디바이스 식별자 개념이 없고, 0943에는 데이터 구조가 없다. 두 표준을 잇는 지점을 `device_install_info` 한 곳으로 제한했다.
...
### 4.3 승인 게이트를 DB 제약으로 구현

기능 3의 핵심 주장은 *"AI 출력이 직접 구동기를 제어하지 않는다"* 이다. 이를 애플리케이션 코드가 아니라 **DB 제약**으로 강제했다.

**① 승인 상태와 규칙 내용이 서로를 구속한다**

```sql
-- control_rule
CHECK ((approved_at IS NULL) = (approved_by IS NULL))       -- 승인시각·승인자 동시
CHECK (approved_at IS NOT NULL                               -- 미승인 → 명령·대상 없음
       OR (action_json IS NULL AND target_install_id IS NULL))
CHECK (approved_at IS NULL                                   -- F-039 · F-049
       OR (action_json IS NOT NULL AND condition_expr IS NOT NULL
           AND target_install_id IS NOT NULL))
```

> **F-039 — 승인 스냅샷은 완결되어야 한다.** 앞의 두 CHECK만으로는 `approved_at`이 채워진 채
> `action_json`이 NULL인 규칙을 만들 수 있었다. 그러면 아래 ③의 명령 대조 트리거가
> `NEW.command_json <> NULL` → SQL NULL로 평가되어 **발화하지 않고**, 임의 명령이 통과한다.
> 세 번째 CHECK가 그 상태 자체를 없앤다.
>
> **F-049 — 승인 스냅샷에는 '어느 장치를'이 포함되어야 한다.** 명령 JSON만 대조하면
> 같은 명령을 유지한 채 실행 레코드의 `install_id`를 다른 구동기로 바꿀 수 있다.
> 0937 6.5는 *"사용자가 지정한 명령을 구동기가 실행하도록"* 이라고 규정한다 — 명령과 대상은
> 분리되지 않는다. 대상을 `target_install_id` 컬럼으로 승격해 DB가 직접 대조하게 했다.
> JSON 안의 문자열은 파싱 없이 비교할 수 없으므로 정규화된 컬럼이어야 한다.
>
> 결과적으로 승인은 `(condition_expr, action_json, target_install_id, approved_at, approved_by)`를
> 한 번에 채우는 **단일 UPDATE**로만 가능하다.

**② 모든 제어 실행에 사람의 권한 출처가 있어야 한다**

`control_rule`만으로는 부족하다. 실행 테이블이 규칙을 우회해 임의 명령을 담을 수 있기 때문이다. `control_execution`에 권한 출처를 명시하도록 했다.
...
| `origin` | 의미 | 표준 근거 |
|---|---|---|
| `RULE` | 승인된 제어 규칙에 근거한 자동 실행 | 0937 부속서 A 3 (자동제어 지원) |
| `MANUAL` | 사용자가 직접 지시 | 0937 부속서 A 1·2 (수동·원격제어 지원) |

**③ 트리거가 승인 여부를 실제로 확인한다**
...
명령 대조 트리거는 **NULL 안전 비교**를 쓴다. `<>`는 한쪽이 NULL이면 결과가 SQL NULL이라 `WHEN`이 참이 되지 않는다(F-039).
...
**④ 승인·거부 기록은 사후에 봉인된다** (F-091: 거부 불변 트리거를 같은 표에 넣었다 — 둘 다 '사람의 결정을 사후 위조하지 못한다'는 같은 주장의 근거다)

| 트리거 | 막는 것 | 발견 |
|---|---|---|
| `trg_rule_action_immutable_after_approval` | 승인된 명령 변조 | F-030 |
| `trg_rule_condition_immutable_after_approval` | 승인된 조건식 변조 — 조건만 넓히면 승인한 명령이 승인하지 않은 상황에서 실행된다 | F-039 |
| `trg_rule_target_immutable_after_approval` | 승인된 대상 장치 변조 | F-049 |
| `trg_rule_approval_irrevocable` | 승인 철회 (`approved_at` → NULL) | — |
| `trg_rule_approver_immutable` | **승인자 위조** — 다른 사용자로 바꿔치기 | F-048 |
| `trg_rule_approved_at_immutable` | **승인시각 위조** | F-048 |
| `trg_exec_rule_immutable` | 실행 레코드의 권한 필드 + `install_id` 사후 변경 | F-017 / F-049 |
| `trg_rule_reject_immutable` | 거부 시각·거부자·**거부 사유** 사후 변경 | F-083 / F-091 |

> **승인자와 승인시각이 곧 감사 기록이다.** 철회만 막고 두 값의 변경을 열어두면
> "누가 언제 최종 결정했는가"를 사후에 위조할 수 있다 — 기능 3의 핵심 주장이
> 증거 수준에서 무너진다(F-048).

> **DB가 보장하는 범위를 정확히 적는다.** 위 제약은 *"제어 실행 레코드는 승인된 규칙 또는 사용자 지시 없이는 존재할 수 없다"* 를 보장한다. 프레임 송신 경로가 DB를 거치지 않고 직접 구동기로 나가는 것까지는 DDL이 막을 수 없으므로, **서비스 계층에서 `control_execution` 기록을 선행 조건으로 강제**하고 그 경로에 회귀 테스트를 둔다.

---

## 5. 표준 내부 불일치 6건 (구현 중 발견)

1369-Part1을 스키마로 직역하는 과정에서 본문·개념모델·논리모델이 서로 어긋나는 지점을 확인했다. **각 항목에 대해 어느 쪽을 채택했는지 근거와 함께 기록한다.** `docs/standard-findings.md`에 기록한다.

| # | 불일치 | 위치 | 채택한 해석 |
|---|---|---|---|
| 1 | **온실정보·장치설치정보의 속성 구성이 그림마다 다르다.** 그림 7-2에는 소유자·관리자 속성이 없고 관계 엔티티(온실소유·온실관리)로만 표현되는데, 그림 7-4에는 `온실 정보`에 `관리자`·`소유자` 속성이, `장치설치 정보`에 `관리자`·`장치상태 데이터 식별자(FK)`가 직접 붙어 있다 | 그림 7-2 vs 그림 7-4 | **그림 7-2 + 본문 7.2.2를 정본으로 채택.** 관계 엔티티가 규범 서술(7.2.2.7~10)에 명시되어 있으므로 FK 중복을 피했다 |
| 2 | **환경상태 데이터 항목 수가 다르다.** 본문 6.3.3.1은 10종(온도·습도·CO2·일사·풍향·**풍속**·감우·토양수분장력·EC·pH)인데 그림 7-3에는 **풍속이 없다**(9종) | 6.3.3.1 vs 그림 7-3 | **본문 10종 채택.** 풍속은 6.3.3.7에 독립 절이 있고 온실 환기 제어에 필요하다. 그림의 누락으로 판단 |
| 3 | **장치상태 데이터 항목 수가 다르다.** 본문 6.3.4.1은 7종(창·보온덮개·**차광막**·환풍기·관수펌프·관수밸브·냉난방기)이라 하는데, 실제 절은 6개(6.3.4.2~6.3.4.7)이고 그림 7-3도 6종이다. **차광막만 항목 목록에 있고 정의가 없다** | 6.3.4.1 vs 6.3.4.2~7 / 그림 7-3 | **6종 채택.** 차광막은 속성 정의가 없어 스키마화 불가. 확장 지점으로만 기록 |
| 4 | **생육작물이 본문에만 있다.** 6.2.3은 *"온실정보에는 … 난방형태, **생육작물**, 작기정보, 활용상태 등이 포함되어야 한다"* 고 하는데, 그림 7-2·7-4의 `온실 정보`에는 작기정보만 있고 생육작물이 없다 | 6.2.3 vs 그림 7-2 / 7-4 | **본문 채택.** `greenhouse_info.crop` 컬럼 추가 |
| 5 | **감우의 단위 취급이 상반된다.** 본문 6.3.3.8은 *"주로 사용하는 감우 데이터의 단위가 관리되어야 한다"* 를 요구하는데, 그림 7-3의 `감우`는 `측정값` 하나만 갖는다 | 6.3.3.8 vs 그림 7-3 | **본문 채택.** 단위는 허용하고 오차범위·유효범위만 금지 |
| 6 | **농장관리 관계가 논리적 모델에 없다.** 7.1(2)는 *"농장 정보는 사용자 정보와 N:1 '농장 관리' 관계를 가진다"* 고 하는데, 7.2.2의 관계 엔티티 4종(온실소유·온실관리·장치설치·장치관리)에 농장관리가 없다 | 7.1(2) vs 7.2.2.7~10 | **논리적 모델 채택.** 7.2.2.2가 `농장정보.소유자`를 FK로 규정하므로 `farm_info.owner_id`로 표현. 별도 관계 테이블은 두지 않음 |

> 본문 6.3.3.1은 `KS X 3269`, 6.3.4.1은 `KS X 3268`을 참고했다고 명시한다. 위 불일치는 두 KS 표준의 항목을 옮기는 과정에서 발생한 것으로 보인다.
...
```
테이블 31개, 트리거 37개, 인덱스 8개 생성 성공
```

### 6.2 2차 — 표준 유래 제약 동작 테스트 (109종, 109/109 통과, 당시 98종 · 2026-08-07, 101종 · 2026-08-10 오전, 103종 · 2026-08-10, 108종 · 2026-08-11)

| 분류 | 테스트 | 조항 |
|---|---|---|
| **식별자 불변** | `farm_info.id` / `greenhouse_info.id` / `device_info.id` / `device_install_info.id` / `user_info.id` UPDATE 차단 | 7.2.2.2~7.2.2.6 |
| | `device_state_data.reported_at` / `env_state_data.measured_at` UPDATE 차단 | 7.2.3.2 / 7.2.3.3 |
| | `farm_info.created_at` UPDATE 차단 | 7.2.2.2 |
| | `device_info.model_name` UPDATE 차단 | 7.2.2.4 |
| | `greenhouse_info.name` UPDATE **허용** (수정 가능 속성) | 7.2.2.3 |
| **관계 유일성** | 온실소유 / 온실관리 / 장치설치 / 장치관리 중복 차단 | 7.2.2.7~7.2.2.10 |
| | 장치상태 / 환경측정 / 온실환경 / 작동환경 중복 차단 | 7.2.4.2~4, 7.2.3.4 |
| | 관계 엔티티 FK UPDATE 차단 | 7.2.4.3 |
| **참조 무결성** | 미존재 장치정보 참조 차단 | 7.2.2.5 |
| | 측정형 데이터 timestamp NOT NULL | 6.3.2 |
| **필수값 보강** (F-158·F-159·F-162·F-166) | `device_info.model_name` 전역 유일성(중복 차단) / 장치설치 설치일자 NOT NULL 차단 / 설치일자 빈 문자열 차단 / 설치일자 형식(ISO 8601) 위반 차단·오프셋 표기 허용 | 6.1 / 6.2.4 / 6.2.5 |
| **시간 형식 확대** (F-184) | 사용자정보 생성·갱신시간 형식 위반 차단 / 삭제시간(nullable) 형식 위반 차단·NULL 허용 / 제어실행 issued_at 형식 위반 차단 | 6.1 |
| **필드 보강** (F-185) | 장치정보 장치특성 컬럼 존재 | 6.2.4 |
| **측정값** | 미정의 환경 subtype 차단 / 미정의 장치 subtype 차단 | 6.3.3.1 / 6.3.4.1 |
| | 감우에 단위 부여 차단, 측정값 단독 허용 | 그림 7-3 |
| **0943 연동** | Node ID 20bit 초과 차단 / Device ID 8bit 초과 차단 | 0943 7.2.4 / 5.1 |
| | `(node_id, device_id)` 중복 차단 | 0943 3.4 |
| **승인 게이트** | 미승인 규칙의 실행명령 차단 / 승인자 없는 승인 차단 | 0937 6.3, 부속서 A 3.3 |
| | 승인된 규칙의 실행명령 허용 / AI 초안 저장 허용 | 0937 6.3, 부속서 A 3.3 |
| | 미승인 규칙 기반 제어 실행 차단 / 권한 출처 없는 실행 차단 | 0937 부속서 A 3.2 |
| | 승인된 규칙 기반 실행 허용 / 사용자 직접 지시 허용 / 승인 철회 차단 | 0937 부속서 A 1·2·3, 6.3 |
| **카디널리티** | 1:N 관계 8종의 단일 부모 조건 (§4.1-a) | 7.1(1)(3)(4)(5)(7)(8)(9)(10) |
| **불변성 보강** | 생성시간 3종 / 관계 FK 5종 / 관계 식별자 3종 | 7.2.2.4~10, 7.2.3.4, 7.2.4.2~4 |
| **명령 결속** | 승인 내용과 다른 명령 차단 / 일치 시 허용 / 승인 후 변조 차단 | 0937 부속서 A 3.2, 6.3 |
| **커버리지** | 생육작물 컬럼 / 감우 단위 허용 / 감우 오차·유효범위 차단 | 6.2.3, 6.3.3.8, 그림 7-3 |
| **승인 스냅샷** | NULL 명령 승인 차단 / NULL 조건식 승인 차단 / 명령 없는 승인 UPDATE 차단 | 0937 부속서 A 3.2 |
| | 원자적 승인 UPDATE 후 실행 허용 / 승인 후 조건식 변조 차단 / NULL 승인 경유 임의 명령 차단 | 0937 부속서 A 3.2 |
| **승인 출처** | 승인자 사후 변조 차단 / 승인시각 사후 변조 차단 | 0937 부속서 A 3.2 |
| **대상 결속** | 승인 대상과 다른 장치 실행 차단 / 일치 시 허용 / 실행 후 대상 바꿔치기 차단 | 0937 6.5 |
| | 승인 후 대상 변조 차단 / 대상 없는 승인 UPDATE 차단 / 미승인 규칙의 대상 확정 차단 | 0937 6.5, 부속서 A 3.3 |
| | MANUAL 실행은 대상 제약 없음 (권한 출처가 사용자 지시) | 0937 부속서 A 1·2 |
| **거부 상태** (F-083) | 거부자 없는 거부 차단 / 사유 포함 거부 허용 / 승인·거부 동시 상태 차단 2종 | 0937 부속서 A 3.2 |
| | 거부 뒤 승인 차단 / 승인 뒤 거부 차단 / 거부 사실 변경 차단 | 0937 부속서 A 3.2 |
| **생성 경로** (F-083·F-091) | AI 초안인데 생성 경로 없음 차단 / 정의되지 않은 생성 경로 차단 / AI+폴백 허용 | 0937 6.3 |
| | **사람 규칙을 AI 산출물로 위조 차단** / 생성 경로가 origin 과 어긋남 차단 / 같은 경로·미기재 허용 | 0937 6.3 |
| **거부 증거** (F-091) | 사유 없는 거부 차단 / 공백뿐인 사유 차단 / 거부 없이 사유만 차단 / 사유 사후 변경 차단 | 0937 부속서 A 3.2 |
| **알림 결속** (F-092) | NEC 알림인데 원본 프레임 없음 차단 / 결속 시 허용 / 임계 알림은 프레임 없이 허용 | 0943 8.2.1.1 |

이 테스트가 `backend/tests/test_schema_conformance.py`의 원형이 된다. **테스트 함수명에 조항 번호를 넣어** 심사자가 목록만 봐도 표준 준수 항목이 보이게 한다.

---

## 7. 다음 단계 (D3)

1. `backend/models.py` — 본 DDL의 행을 담는 **읽기 전용 dataclass** (ORM 미사용. CLAUDE.md §4.3)
   `backend/repository.py` — SQL 담당. `schema.sql`이 정본이고 트리거·CHECK가 여기 있다
2. `backend/ingest.py` — `Frame` → 테이블 매핑 규칙 구현
   - `REQ_SET_CONNECTION` → `device_install_info` + `device_install` (기능 1)
   - `NOTI_DEVICE_VALUE` → `env_state_data` + `env_measurement` + `env_measure` (FMS)
   - `NOTI_ERROR` → `alert` (FCS)
   - `NOTI_DISCONNECT` → `alert` (FCS) — 단계 6 F-191에서 결선(당초 이 목록에 없었다)
   - 모든 프레임 → `frame_log` (+ 위반 시 `frame_violation`)
3. `backend/tests/test_schema_conformance.py` — 본 문서 §6.2를 pytest로 이식
4. 시드 데이터 — 농장 1 / 온실 1 / 사용자 1 (데모 고정값)
