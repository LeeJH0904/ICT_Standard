...
**`개발_운영_가이드.md` 와 `GPT_검증_프롬프트.md` 는 사람용이므로 읽지 않는다.**
...
| `project_docs/dev/GPT_검증_프롬프트.md` | 단계별 GPT Codex 검증 프롬프트. 정본은 `GPT.md` · `fix_log/README.md` 이며 이 문서는 호출 방법만 정한다 |
...
## 11. fix_log 처리 — GPT 검증 결과 반영

GPT가 보조 검증자로 코드·문서를 점검하고 발견 사항을 `fix_log/`에 기록한다. **기록 규약은 `fix_log/README.md`가 정본이다.** 역할 지시서는 `GPT.md`.
...
**상태를 바꾸는 것은 Claude뿐이다.** GPT는 `신규`로만 추가한다.
...
**인덱스의 행을 삭제하지 않는다.** 상태만 갱신한다 — 기록이 남아야 GPT가 같은 지적을 반복하지 않는다.
